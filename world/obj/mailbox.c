...
if(sizeof(mails)>29)
return notify_fail("你的信箱已经塞满了，删掉点再写吧。\n");
if((time()-this_player()->query("last_mail"))<40)
return notify_fail("写信也不用那么急啊，等等再写吧。\n"); 
...
void get_mail_title(string str, mapping mail)
{
    ...
    str=CONVERT_D->input(str, this_player());
        ...
        write("信件内容：\n");
  this_player()->set("last_mail",time());
        this_player()->edit( (: get_mail_text, mail :) );
}
void get_mail_text(mapping mail, string str)
{
        mail["text"] = str;
    write("将会给您自己留一份备份，按回车键继续。");
...
if((time()-this_player()->query("last_mail"))<10)
return notify_fail("转信也不用那么急啊，等等再转吧。\n"); 
