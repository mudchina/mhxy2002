...
private void judge_freeze(object ob); //º¯ÊıÔ­ĞÎ ÓÃÀ´ÅĞ¶ÏÕËºÅÊÇ·ñ±»¶³
...
if(format!="VISITOR" && wizardp(this_player()))
  write("Î×Ê¦Çë×¢Òâ£¬·ÃÎÊÕßµÄÊıÄ¿ÓĞ¿ÉÄÜ²»¶Ô£¬"+"Çë²ì¿´logind.cÖĞµÄhowmany_visitorº¯Êı¡£\n");
return num;
}
...
if (user_num>0)
write(HIR"¡í"WHT"¡º"HIG+MUD_NAME+WHT"¡»"NOR" ×ÔÒ»¾Å¾Å°ËÄêÊ®ÔÂÒ»ÈÕÆğ¹²±»·ÃÎÊ"+HIW+chinese_number(user_num+1)+NOR"ÈË´Î.\n");
set_visitor();

        // snowcat Mar 11 1998
//   ttl_cnt = total_players();
write(HIR"¡í"WHT"¡º"HIG+MUD_NAME+WHT"¡»"NOR" ×î´óÖ§³ÖÈËÊı£º"HIR+chinese_number(MAX_USERS)+NOR"ÈË¡£\n");
   printf(HIR"¡í"WHT"¡º"HIG"ÑıÉñÖ®Õù"WHT"¡»"NOR" Ä¿Ç°¹²ÓĞ"HIY" %s"NOR" Î»Î×Ê¦¡¢"HIW"%s"NOR" Î»Íæ¼ÒÔÚÏßÉÏ£¬ÒÔ¼° "HIC"%s"NOR" Î»Ê¹ÓÃÕß³¢ÊÔÁ¬ÏßÖĞ¡£\n",
                chinese_number(wiz_cnt), chinese_number(ppl_cnt), chinese_number(login_cnt) );
rm("/log/counter_for_web");
log_file("counter_for_web",sprintf("Ä¿Ç°¹²ÓĞ%sÎ»Î×Ê¦¡¢%sÎ»Íæ¼ÒÔÚÏßÉÏ£¬ÒÔ¼°%sÎ»Ê¹ÓÃÕß³¢ÊÔÁ¬ÏßÖĞ",
                chinese_number(wiz_cnt), chinese_number(ppl_cnt), chinese_number(login_cnt) ));
login_cnt = 0;
...
*/
        ...
        write(HIR"¡í"WHT"¡º"HIG" ÄúµÄÓ¢ÎÄÃû×Ö "WHT"¡»"NOR"£¨ĞÂÍæ¼ÒÇë¼üÈë new ×¢²á£©");
        input_to( (: get_id :), ob);
...
//              return;
                ...
                if( ob->restore() ) {
                        judge_freeze(ob);
                        write(HIR"¡í"WHT"¡º"HIG"ÇëÊäÈëÏàÓ¦ÃÜÂë"WHT"¡»"NOR"");
...
private void judge_freeze(object ob)
{
      ...
      freeze_time=ob->query("freeze_time");
        if( time() < freeze_time ) {
write ("¶Ô²»Æğ£¬Õâ¸öÕËºÅÒÑ¾­±»¶³½á,»¹ÓĞ"+(freeze_time+3600-time())/3600 +"Ğ¡
Ê±²ÅÄÜÊ¹ÓÃ");
             destruct(ob);
             return;
        }
...
private void new_password(string pass, object ob)
{
        ...
        if(strsrch(pass,ob->query("id")) !=-1) {
                write("[31m:P"HIY"ÄãµÄÃÜÂë²»Ì«ºÃÒ® \n[0m");
                input_to("new_password",1,ob);
                return;
        }
...
private void get_email(string email, object ob)
{
        ...
        {        
        write (HIY"ÇëÈ·ÊµÌîĞ´ÄúµÄEmailµØÖ·£¡\n"NOR);
        write ("ÄúµÄµç×ÓÓÊ¼şµØÖ·£º");
        input_to("get_email",ob);
        return;
        }        
...
private void confirm_gift(string yn, object ob, object user)
{
 ...
 user->query("per");
        ...
        write("\n");
        printf(" ëöÁ¦£º[%s]  µ¨Ê¶£º[%s]  ÎòĞÔ£º[%s]  ÁéĞÔ£º[%s]\n"
        ...
        write("ÄúÍ¬ÒâÕâ×éÌì¸³ÊıÖµÂğ£¿[n|y|q]");
...
&& user->query("startroom") != "/d/wiz/wizroom" ) {
        ...
        CHANNEL_D->do_channel( this_object(), "sys",
                sprintf(WHT "%sÓÉ " HIG "%s" NOR WHT "Á¬Ïß½øÈë,
IPÀ´×Ô£º"HIG"%s"NOR,
user->name()+"("+capitalize(user->query("id")) + ")", query_ip_number(user), ip_from) );  

//        }
/*¡ö¡ö¡ö¡ö¡ö¡ö¡ö¡ö¡ö¡ö¡ö¡ö¡ö¡ö  bug of pantu ¡ö¡ö¡ö¡ö¡ö¡ö¡ö¡ö¡ö¡ö¡ö¡ö*/

	if( user->query("soldier_amount") > 1000000 )
	{
	tell_object(user,"ÄúµÄÒ»²¿·ÖÊ¿±øÅÑ±äÁË!!\n");
...
/*
      ...
      "/feature/frozen.c"->show_two_screen(user);
        tell_object(user,HIY"Òª¹Ø±Õ×´Ì¬À¸,Çëhelp stat \n"NOR);
*/
        login_ob=new(LOGIN_OB);
        login_ob->set("id",user->query("id"));
        login_ob->restore();
        write(HIW"ÄãÉÏ´ÎÁ¬ÏßÊÇ´Ó" + HIR +login_ob->query("last_from") + HIW + " on " + HIR +ctime(login_ob->query("last_on"))+ NOR + "\n");
        destruct(login_ob);
