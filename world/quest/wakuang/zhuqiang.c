...
int do_zhuqiang(string arg);
 ...
 void create()
{
        set("short", WHT"铸枪房"NOR);
        set("long", @LONG
这是一个很平常普通的铸枪房！四周都是乱七八糟的铜皮铁块，人们
把捡来的矿石在这儿炼化以后铸造成各种名枪。
LONG
        );
...
int do_zhuqiang(object ob)
{
        ...
        if (!(ob = present("tong kuang", this_player())) &&
             ...
             !(ob = present("han tie", this_player())) )
                return notify_fail("你身上没有矿石，如何铸枪？\n");
                 
        if ( (int)me->query("sen")<250)
                return notify_fail("你的精力不足，无法铸枪。\n");
        if( me->is_busy() )
                return notify_fail("你现在很忙！不能铸枪！\n");

        if ( (int)me->query("kee")<250)
                return notify_fail("你的体力不足，无法铸枪。\n");

        if ( (int)me->query("force")<300)
                return notify_fail("你的内力不足，无法铸枪。\n");

        message_vision(HIY"$N先将矿石放进炉中炼化成块状状。\n"NOR,me); 
        message_vision(HIY"接着$N再拿出一个大锤开始铸枪。\n"NOR,me); 
        message_vision(HIY"不一会儿！一把上好的铁枪摆在了$N的面前。\n"NOR,me); 
...
//        me->add("daoxing",50+random(50));
        ...
        return notify_fail("由于你铸枪之技太低，枪根本不能用！\n");
        }
