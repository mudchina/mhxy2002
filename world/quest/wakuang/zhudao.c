...
int do_zhujia(string arg);
 ...
 void create()
{
        set("short", HBRED+HIW"铸刀房"NOR);
        set("long", @LONG
这是一个很平常普通的铸刀房！四周都是乱七八糟的铜皮铁块，人们
把捡来的矿石在这儿炼化以后铸造成各种名刀。
LONG
        );

        set("exits", ([
                "west" : __DIR__"zhujian",
                "south" : __DIR__"zhucha",
        ]));
        set("no_fight",1);
        set("no_beg",1);
        set("no_steal",1);
...
int do_zhujia(object ob)
{
        ...
        int b1;
        me=this_player();
        b1 = this_player()->query("work/zhudao");
        ...
        if (!(ob = present("tong kuang", this_player())) &&
             ...
             !(ob = present("han tie", this_player())) )
                return notify_fail("你身上没有矿石，如何铸刀？\n");
                 
        if ( (int)me->query("sen")<200)
                return notify_fail("你的精力不足，无法铸刀。\n");

        if( me->is_busy() )
                return notify_fail("你现在很忙！不能铸刀！\n");

        if ( (int)me->query("kee")<200)
                return notify_fail("你的体力不足，无法铸刀。\n");

        if ( (int)me->query("force")<300)
                return notify_fail("你的内力不足，无法铸刀。\n");

        message_vision(HIY"$N先将矿石放进炉中炼化成块状状。\n"NOR,me); 
        message_vision(HIY"接着$N再拿出一个大锤开始铸刀。\n"NOR,me); 
        message_vision(HIY"不一会儿！一把锋利的刀摆在了$N的面前。\n"NOR,me); 
...
//        me->add("daoxing",50+random(50));
        ...
        {
        destruct(ob);             
        ob1=new(__DIR__"weapon/dao/dao1");
        ob1->set("value", 250);
        ob1->move(me);
        me->add("work/zhudao", 7);       
        return 1;
        }
        ...
        if ( b1 < 100 ) 
        {
        me->add("work/zhudao",6);
        return notify_fail("由于你铸刀之技太低，刀根本不能用！\n");
        }
        ob1=new(__DIR__"weapon/dao/dao2");
        ob1->set("value", 0);
        ob1->set("no_get", 1);
        ob1->set("no_sell", 1);
        ob1->set("no_put", 1);
        ...
        if ( b1 < 300 )
        {
        me->add("work/zhudao",5);
        return notify_fail("由于你铸刀之技太低，刀根本不能用！\n");
        }
        ob1=new(__DIR__"weapon/dao/dao3");
        ob1->set("value", 0);
        ob1->set("no_get", 1);
        ob1->set("no_sell", 1);
        ob1->set("no_put", 1);
        ...
        if ( b1 < 500 )
        {
        me->add("work/zhudao",4);
        return notify_fail("由于你铸刀之技太低，刀根本不能用！\n");
        }
        ob1=new(__DIR__"weapon/dao/dao4");
        ob1->set("value", 0);
        ob1->set("no_get", 1);
        ob1->set("no_sell", 1);
        ob1->set("no_put", 1);
        ...
        ob1=new(__DIR__"weapon/dao/dao5");
        ob1->set("value", 0);
        ob1->set("no_get", 1);
        ob1->set("no_sell", 1);
        ob1->set("no_put", 1);
        ...
        ob1=new(__DIR__"weapon/dao/dao6");
        ob1->set("value", 0);
        ob1->set("no_get", 1);
        ob1->set("no_sell", 1);
        ob1->set("no_put", 1);
        ...
        ob1=new(__DIR__"weapon/dao/dao7");
        if ( b1 > 5000 )
        {
        ob1=new(__DIR__"weapon/dao/dao8");
        }
        ob1->set("value", 0);
        ob1->set("no_get", 1);
        ob1->set("no_sell", 1);
...
{
        write(this_player()->query("name")+"，安心的铸刀吧！\n");
        return 1;
}
