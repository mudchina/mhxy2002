...
int do_zhujia(string arg);
 ...
 void create()
{
        set("short", HIR"铸杖房"NOR);
        set("long", @LONG
这是一个很平常普通的铸杖房！四周都是乱七八糟的铜皮铁块，人们
把捡来的矿石在这儿炼化以后铸造成各种名杖。
LONG
        );
...
int do_zhujia(object ob)
{
        ...
        if (!(ob = present("tong kuang", this_player())) &&
             ...
             !(ob = present("han tie", this_player())) )
                return notify_fail("你身上没有矿石，如何铸杖？\n");
                 
        if ( (int)me->query("sen")<250)
                return notify_fail("你的精力不足，无法铸杖。\n");

        if( me->is_busy() )
                return notify_fail("你现在很忙！不能铸杖！\n");

        if ( (int)me->query("kee")<250)
                return notify_fail("你的体力不足，无法铸杖。\n");

        if ( (int)me->query("force")<300)
                return notify_fail("你的内力不足，无法铸杖。\n");

        message_vision(HIY"$N先将矿石放进炉中炼化成块状状。\n"NOR,me); 
        message_vision(HIY"接着$N再拿出一个大锤开始铸杖。\n"NOR,me); 
        message_vision(HIY"不一会儿！一把上好的杖摆在了$N的面前。\n"NOR,me); 
...
//        me->add("daoxing",50+random(50));
        ...
        return notify_fail("由于你铸杖之技太低，杖根本不能用！\n");
        }
...
{
        write(this_player()->query("name")+"，安心的铸杖吧！\n");
        return 1;
}
