...
int main(object me, string arg)
{
	...
	string dirname="/data/wiz_doc/", deloption, srcstr, dststr, file;
	...
	if ( argn >3)
		if (deloption=="-delete")
			...
			return notify_fail("指令格式：doc [<month> [<number>]] [<year>]\n");
...
+"篇文档：\n--------------------------------------------------------------------------------\n";
	...
	if (deloption=="-delete") {
		...
		rm(dirname+sprintf("%s/doc%d.%d.o",mon[month-1],month,nummonth));
		write("文档删除成功！\n");
		return 1;
		}
...
{
        write(@HELP
指令格式：doc <month> [<number> [<year>]]

这个指令让你在线阅读保存的巫师留言版文档，或者列出
所能读取的文档目录。
