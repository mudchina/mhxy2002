...
int main(object me, string arg)
{
        ...
        if( !arg || sscanf(arg, "%s %s", arg, mission)!=2 )
                return notify_fail("指令格式：renwu <巫师名> (目前区域)\n");
        if( !objectp(ob = find_living(arg)) || !userp(ob) )
                return notify_fail("你只能改变当前使用者的任务。\n");
        mission = replace_color( mission, 1);
          wmission = explode(read_file("/adm/etc/job"), "\n");
        ...
        for( file = "", i=0; i<sizeof(wmission); i++) file += wmission[i] + "\n";
         if(!flag) write_file("/adm/etc/job", ob->query("id") + " " + mission + "\n", 0);
          else      write_file("/adm/etc/job", file, 1);
        message_vision("$N将$n的工作区域改为 " + mission + " 。\n", me, ob);
        return 1;
}
