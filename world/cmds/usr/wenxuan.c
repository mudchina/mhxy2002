...
int main(object me, string arg)
{
	...
	else if (sscanf(arg, "del %d %d",number,j)>=1){
		if (!wizardp(me)) return notify_fail("对不起，不是巫师不能删除文选。\n");

		if (sizeof(get_dir("/data/wenxuan/"+(j?j:y)+"/"))<number||number<1)
			return notify_fail("没有这篇文选。\n");
		write("正在删除/data/wenxuan/"+(j?j:y)+"/"+number+".w 文件……");
		if (rm("/data/wenxuan/"+(j?j:y)+"/"+number+".w")) {
			write("……成功。\n");
			return 1;
		}
		else return notify_fail("……失败！\n");
	}
...
wenxuan add <留言序号>       : 添加一篇文选（巫师和版主负责）
wenxuan del <留言序号> <年份>: 删除一篇文选（巫师负责）
