...
int give_reward(object me,mapping quest)
{
  ...
  me->add("combat_exp",4*bonus/5);
    ...
    me->add( "faith", 1);
    me->add( "weiwang", 1);
    tell_object(me,HIW"你的师门威望和忠诚度升高了。你还得到了"+chinese_number(bonus*3/4)+
    ...
    if(num==10){
        ...
        tell_object(me,HIW"你得到了"HIY+chinese_number(500)+HIR
                "点实战经验"HIW"和"HIY+chinese_number(200)+HIR"点潜能"HIW"的附加奖励。\n"NOR);
    }             
...
int main(object me, string arg)
{
  ...
  mapping quest;
        ...
        if(me->is_apprentice_of(inv[i])){
           if(arg=="cancel"){
                if( me->query("faith")<50 )
                 return notify_fail(HIR"你师傅现在还不是很信任你，还是去做吧。\n"NOR);
                else
                me->add("faith",-2);
                me->delete_temp("quest_number");
                me->delete("fy_quest");
    ...
    write(WHT"你现在的任务是" +HIY "〖"HIR + quest["quest"] +HIY "〗。\n"NOR);
    nowtime = (int) me->query("task_time") - time();
    if( nowtime  > 0 )
                time_period(nowtime, me);
        else
                write(RED"但是你已经"WHT"没有足够"RED"的时间来完成它了。\n"NOR);    
    return 1;
}
