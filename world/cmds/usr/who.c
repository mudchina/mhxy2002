...
mixed main(object me, string arg, int remote)
{
    ...
    if(me) if(objectp(me)) if( !wizardp(me))
        ...
        str = NOR+YEL"☆ "HIY"释厄传"HIR"「梦幻西游」"NOR+HIW;
        if(opt_banghui)
                if(me->query("banghui"))
...
//                        if(list[i]->query("id")=="tianlin") continue;
        ...
        str += HIC"═════════════════════════════════════════════\n"NOR;
        if(opt_number)
                str = HIY"「"+ CHINESE_MUD_NAME+"」"NOR + WHT"目前当前共有 "HIC+(player_cnt)+NOR+WHT" 位使用者连线中。\n";
        else if(player_cnt)
...
"HIM"*"NOR" 表示"HIM"发呆"NOR"中，"HIG"#"NOR" 表示"HIG"断线"NOR"中。
