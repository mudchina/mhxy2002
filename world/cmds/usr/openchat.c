...
int vote(object me, object victim)
{
 ...
 if(victim->query("chblk_sldh"))
  ...
  } else if (victim->query("chblk_rumor")) 
  {
   ...
   else if(victim->query("chblk_music"))
  {
	channel = "心笙乐府";
  }
   else if(victim->query("chblk_jh"))
  {
	channel = "江湖同盟";
  }
   else if(victim->query("chblk_cht"))
  {
	channel = "传呼台";
  }
  ...
  } else 
  {
	...
	victim->delete("chblk_chat");
       victim->delete("chblk_cht");//这是我加入的
       victim->delete("chblk_music");//
       victim->delete("chblk_jh");//目前有这三个
