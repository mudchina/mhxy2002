...
// 也更有条理。玩家阅读起来也比较方便。
//By alone@yszz.sh 2001/01/10 优化
//By tianlin@mhxy for 2002.1.20,为mhxy修改
#include <ansi.h>;
...
int check_news(object me)
{
        ...
        if(userp(me))
        {
                ...
                if(i < 6 && i > 0) // 小于6条新闻才显示标题，不然会FLOOD
                {
                        ...
                        msg += "                                                  "HIM"【"HIW"梦幻西游"HIY" 巫师组"HIM"】"NOR"\n";
                        msg += HIB"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"NOR;
               ...
               i=sizeof(notes)-me->query("LastReadNews");
                ...
                write(HIG"\n从您上次阅读到现在"NOR+((i==0)? ""HIG"没有任何":""HIG"共有"+HIY+chinese_number(i)+NOR+HIG"条")+HIG"新闻。\n\n"NOR);
                if(i>10) write("你可以 Help news 来查看梦幻西游新闻系统的使用方法。\n");
...
int main(object me, string arg)
{
        ...
        if( !pointerp(notes) || !sizeof(notes) )
                return notify_fail(HIC"梦幻西游 目前没有任何新闻发布。\n"NOR);
                
        if( !arg ) 
        {
                if(i>10) write(HIC"你可以"HIR" Help news "HIC"来查看梦幻西游 新闻系统的使用方法。\n"NOR); //10
                        return notify_fail(
"梦幻西游 目前共有"+HIW+chinese_number(sizeof(notes))+NOR"条新闻"+((i==0)? "。":"，您有"+HIY+chinese_number(i)+NOR"条新闻尚未阅读。")+"\n"); }
        if (sscanf(arg,"add %s",title)==1) {
                if (!title) return notify_fail("请指定一个新闻的标题。\n");
                if (!wizardp(me)) return notify_fail("对不起，只有梦幻西游的巫师才可以发布新闻。\n");
                else {
                        do_post(title);
                        return 1;
                        }
        ...
        if (sscanf(arg,"del %d",number)) {
                if (!number) return notify_fail("你要删除哪一个编号的新闻？\n");
                if (!wizardp(me)) return notify_fail("对不起，只有梦幻西游的巫师才可以删除新闻。\n");
                else {
                        do_discard(number);
                        return 1;
                } //fixed by LitChi
        ...
        CYN"-------------------------------------------------------------------------------\n"NOR,
                ""HIM"【"HIW"梦幻西游"HIM"】"HIC"第"HIR+chinese_number(num + 1)+HIC"条新闻 \n"NOR, 
...
void done_post(object me, mapping note, string text)
{
        ...
        restore();
// 使用channel显示提示有了最新新闻
message("channel:chat", HIM"【"HIW"新闻发布"HIM"】："HIC"梦幻"HIW"西游"HIY"有了最新更新新闻，请用 "HIR"news"NOR HIY" 查看！！！ \n"NOR, users());
                          //By waiwai@mszj 2000/11/13 
