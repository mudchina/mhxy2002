...
string *msg_dunno = ({
	...
	"$nÀäÀäµØËµµÀ£ºÎÒÊ²Ã´¶¼²»ÖªµÀ£¬ÄãÒ²²»ÒªÔÙÎÊÁË£¬¾ÍËãÎÒÖªµÀÎÒÒ²²»»á¸æËßÄãµÄ¡£\n",
	"$nÂŞàÂµØËµµÀ£ºÄãÓĞÊÂÎÊÎÒÂğ£¬ÄãÓĞÊÂ¾ÍÎÊ°É£¬Äã²»ÎÊÎÒÎÒÔõÃ´ÖªµÀÄãÒªÎÊÎÒÊ²Ã´ÄØ¡£\n",
	"$nÂŞàÂµØËµµÀ£ºÄãÖª²»ÖªµÀÊ²Ã´ÊÇîõîõîõîõîõîõ£¿\n",
	"$nËµµÀ£ºÄãÊÇ¸ö´ÏÃ÷ÈË£¬ÎÒ¿ÉÒÔÓÃ´ÏÃ÷µÄ·½·¨¸úÈËËµ»°¡£ÍâÃæµÄÈË¾Í²»ĞĞ£¡\n"
});
...
int main(object me, string arg)
{
	...
	if( !ob->is_character() ) {
		message_vision( YEL "$N¶Ô×Å$n×ÔÑÔ×ÔÓï£®£®£®\n" NOR, me, ob);
		return 1;
	}

	if( !ob->query("can_speak") ) {
		message_vision("$NÏò$n´òÌıÓĞ¹Ø¡º"+HIG"" + topic 
			+ ""+NOR"¡»µÄÏûÏ¢£¬µ«ÊÇ$pÏÔÈ»Ìı²»¶®ÈË»°¡£\n", me, ob);
		return 1;
	}
	...
	if( !INQUIRY_D->parse_inquiry(me, ob, topic) )
		...
		if(topic=="all")
			{
			message_vision( WHT "$N"NOR+WHT"¶ÔÄãÇÄÉùËµµÀ:"+query_inquiry(ob)+"!\n" NOR, ob);
			return 1;
		}
		if(topic=="news")
			{
		...
		if(topic=="here")
			{
			message_vision( CYN "$N¶ÔÄãÇÄÉùËµµÀ:ÕâÀï¾ÍÊÇ"HIW+environment(me)->query("short")+NOR CYN",ÄãÃ»ÌıËµ¹ıÂğ£¿\n" NOR, ob);
			return 1;
		}
		if(topic=="name")
			{
			message_vision( CYN "$NËµµÀ£ºÎÒµÄÃû×Ö¾ÍÊÇ$N," + RANK_D->query_respect(me)+ "ÓĞÊ²Ã´ÊÂ?\n" NOR, ob);
			return 1;
		}
	...
	// by snowcat jan 23 1998
	  if ( msg = QUEST->quest_ask (me, ob, topic) ) {
		if( stringp(msg) ) {
			message_vision( WHT "$NËµµÀ£º" + msg + "\n" NOR, ob);
			return 1;
		}
	}
...
string query_inquiry(object ob)
{
	...
	if(mapp(inq=ob->query("inquiry")))
		...
		str="ÓĞ¹ØÓÚ "NOR+HIR+str+NOR WHT"µÄÊÂÇé,ÎÒºÜ¸ßĞË¸æËßÄã!";
		return str;
		}
