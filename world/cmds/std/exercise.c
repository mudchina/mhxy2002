...
//                remove_call_out("exec_int");
        ...
        if( kee_cost < 100 ) return notify_fail("你最少要花 100 点「气」才能练功。\n");

        if( (int)me->query("kee") < kee_cost )
                return notify_fail("你现在的气太少了，无法产生内息运行全身经脉。\n");

        force_name = to_chinese(force_name);
        write(HBRED+HIW"你运起" + HIC + force_name + NOR + HBRED+HIW + "，一股内息自丹田涌上，开始在体内流动。\n"NOR);

        busy_time=kee_cost/100;
...
//              remove_call_out("exec_int");
        ...
        if (force_gain < 25) force_gain = 25;
        if (force_gain > 200) force_gain = 200;
