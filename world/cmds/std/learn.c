// By tianlin 2001.5.1
// learn.c
//By tianlin@mhxy for 2002.1.21,突发奇想,设定杀过人的player不能学功夫,and练功夫
#include <ansi.h>
#include <skill.h>
...
int main(object me, string arg)
{
        ...
        if( me->query("PKS")>0 )
                return notify_fail("你刚杀过人，没法安心学习。\n");
...
//              && getuid(ob)!=me->query("bonze/dadangid") ) {
    ...
    if (getuid(ob) == me->query("couple/id") && !userp(ob) )
       ...
       /* if(userp(ob) && (int)my_skill<1) {
        ...
        tell_object(ob, sprintf("%s向你请教有关"HIC"「%s」"NOR"的问题。\n",
                ...
                &&      my_skill * my_skill * my_skill / 10 > (int)me->query("daoxing") ) {
                     ...
                     if( my_skill < master_skill ) {
                        ...
                        if(qlearned<me->query_int())
                          message_vision(HIY"$N对$n说道："+
                            "你的「"+to_chinese(skill)+
                            "」已经不输为师了，咱们一起切磋切磋吧！"NOR
                            +"\n",ob,me);
