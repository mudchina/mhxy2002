...
string chinese_time(int type,string get_time)
{
        ...
        if (type) {
                switch( type ) {
                        ...
                        case 1: return year+"Äê"+(member_array(month, month_name) + 1)+
				"ÔÂ"+day+"ÈÕ"+hour+"µã"+minute+"·Ö"+second+"Ãë";
...
int look_room(object me, object env)
{
	...
	if( mapp(exits = env->query("exits")) ) {
		...
		if (env->query("no_look") && !wizardp(me))
			...
			str += "    ÕâÀïÎ¨Ò»µÄ³ö¿ÚÊÇ" + YEL + dirs[0] + NOR;
	...
	//		str += sprintf("    ÕâÀïÃ÷ÏÔµÄ³ö¿ÚÊÇ " + YEL + "%s" + NOR + " ºÍ " + YEL + "%s" + NOR,
...
//By tianlin@mhxy for 2002.1.10.´ÓĞÂ¸Ä±à¸ñÊ½
 //By waiwai@mszj 2000/11/03
   if(me->query("look")=="y"){
	  // str3 =HIW+"- "+chinese_time(6,(string)localtime(time()))+" "+NOR;
	if(( me->query("water") > 0 ) || ( me->query("food") > 0 )){
          str3 +=HIY"\n<"NOR CYN"ÆøÑª:"HIC+me->query("kee")+"/"+me->query("max_kee")+HIY"> ";
          str3 +=HIY"<"NOR CYN"¾«Éñ:"HIC+me->query("sen")+"/"+me->query("max_sen")+HIY"> ";
          str3 +=HIY"<"NOR CYN"ÄÚÁ¦:"HIC+me->query("force")+"/"+me->query("max_force")+HIY"> ";
          str3 +=HIY"<"NOR CYN"·¨Á¦:"HIC+me->query("mana")+"/"+me->query("max_mana")+HIY"> \n"NOR;
          }
      else
	if(( me->query("water") <= 0 ) || ( me->query("food") <= 0 )){
          str3 +=HIY"\n<"NOR CYN"ÆøÑª:"HIC+me->query("kee")+"/"+me->query("max_kee")+HIY"> ";
          ...
          str3 +=HIY"<"NOR CYN"ÄÚÁ¦:"HIC+me->query("force")+"/"+me->query("max_force")+HIY"> ";
          str3 +=HIY"<"NOR CYN"·¨Á¦:"HIC+me->query("mana")+"/"+me->query("max_mana")+HIY"> ";
          str3 +=HIY"<"NOR HIB"¿Ú¿Ê"+HIY"> ";
          str3 +=HIY"<"NOR WHT"¼¢¶ö"+HIY"> \n"NOR;
}}
	write(str+str2+str3);
...
//return special looking first if he/she has one.
	...
	if (ridemsg)
        ...
        if((wearornot == 0 ) && userp(obj) && !wizardp(obj) )
            str += pro+HIY"È«ÉíÉÏÏÂÒ»Ë¿²»¹Ò, ÄÑµÀÏëÂã±¼¡«¡«¡«¡«\n";
        //By waiwai@mszj 2000/11/03
	if( sizeof(inv) ) {
	...
	&&	random((int)obj->query("bellicosity")/10) > (int)me->query_per() ) {
		write( HIW+obj->name() +HIW "Í»È»×ª¹ıÍ·À´µÉÄãÒ»ÑÛ¡£\n"NOR);
