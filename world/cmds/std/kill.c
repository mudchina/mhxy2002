...
int main(object me, string arg)
{
        ...
        if( !obj->is_character() || obj->is_corpse() )
                return notify_fail(CYN"看清楚一点，那并不是活物。\n"NOR);

        if(wizardp(obj)&&!wizardp(me))//玩家不能对wiz下kill指令,但wiz除外 
                return notify_fail(""HIY"玩家"NOR"不能对"HIC"巫师"NOR"下"HIR"Kill"NOR"指令!!\n");

        if (obj->query_temp("netdead")&&!wizardp(me))//这句话是玩家不能对断线的人进行战斗wiz除外
                return notify_fail("你不能对"HIG"断线"NOR"的"HIY"玩家"NOR"进行"HIR"战斗"NOR"！\n");

	 if(userp(obj) && obj->query("age")<16)
		  return notify_fail("上天有好生只德,就放过小孩子吧。\n");

if(me->query("combat_exp")<100000 && userp(obj))//这句话是小于50000 wx的人不能杀人
		  return notify_fail("自己功夫没练好就想杀人？\n");

	 if(me->query("daoxing")<100000 && userp(obj))//这句话是小于50000 dx的人不能杀人
		  return notify_fail("自己功夫没练好就想杀人？\n");	

        if(obj==me)//这句话是如果对自己使用
