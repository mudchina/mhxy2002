...
void create()
{
        set_name(HIB"魔礼海"NOR, ({ "tian wang", "duowen tianwang", "hai", "tianwang" }) );
        set("gender", "男性" );
        set("long",HIC
...
void pipa (object me, object ob)
{
  ...
  message_vision ("$N"HIY"掏出碧玉琵琶，竟在战场上悠然自得的弹了起来～～～～\n"NOR,me);

  if ( (ob->query("mana")+random(1000)) > 3500 )
     message_vision (HIW"$N"HIC"充耳不闻，不为所动。\n"NOR,ob);
  else 
    {
     message_vision (HIW"$N"HIY"就象着了魔一样，顿感头昏脑涨！\n"NOR,ob);
