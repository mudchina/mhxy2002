...
void create ()
{
  ...
  set ("long", @LONG
             ...
             小鸡们的文化比赛就在这举行,如果你要查询上次比赛结果,请输入last.


LONG);

  set("exits", ([ /* sizeof() == 4 */
        "west": __DIR__"baoming",
]));
...
int do_last()
{
   ...
   if( file_size("/log/hen_game2") <=0 )

                return notify_fail("暂时没有比赛结果.\n");

  if( this_player()->query_temp("hen/game_2") )
...
void begin()
{
        ...
        {
      if( present("zhandou ji",list[i]) && list[i]->query_temp("hen/game_2") )
        gamer+=({list[i]});
        }
        ...
        for(n=0;n<sizeof(gamer);n++)
        {
                ...
                {
               if( gamer[x-1]->query("hen/might") > gamer[x]->query("hen/might") )
                        {
                        temp = gamer[x-1];
                   gamer[x-1] = gamer[x];
                   gamer[x] = temp;
         ...
         {
                     log_file("hen_game2",sprintf("%-12s[%-10s]在小鸡文化比赛中获得了第"+HIR
