...
{
	message("shout", HIC"〖"MAG"月宫仙子"HIC"〗嫦娥(Chang e)："HIY"今日是本门"NOR+MAG"广寒宫传人"NOR+YEL+this_player()->name()+HIY"上任的日子，还请诸位神仙多多捧场。\n"NOR,users());
...
int rank_me()
{
	...
	exp=(int)me->query("combat_exp");
        if( me->query("title") == WHT"广寒宫"MAG"传人"NOR)
		return notify_fail("你已经是掌门身份，还来要什么职位．\n");
	if( me->query("family/family_name") == "月宫")
	{
                if( exp <= 20000 ){
			me->set("title", YEL"月宫入门弟子"NOR);
		} else if(exp <= 150000 ){
			me->set("title", HIG"月宫锄草仙子"NOR);
		} else if(exp < 500000 ){
			me->set("title", WHT"月宫种花仙子"NOR);
		} else if(exp <= 800000 ){
			me->set("title", HIR"月宫掌药玉女"NOR);
		} else if(exp <= 1500000 ){
			me->set("title", RED"月宫护花使者"NOR);
		} else if(exp <= 3000000 ){
			me->set("title", HIM"月宫总管"NOR);
		} else if(exp <= 5000000 ){
			me->set("title", MAG"百花仙子"NOR);
		} else {
			me->set("title", MAG"似水红颜惹人怜"NOR);
		}
