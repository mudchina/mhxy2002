//nuwa.c
//By tianlin@Mhxy for 2002.1.24.¿ªÍ¨ºÍÓÅ»¯
#include <ansi.h>
inherit NPC;
inherit F_MASTER;
#include <obstacle.h>
...
{        	command("pat "+ob->query("id"));
		...
		if (ob->query("title")=="¹ãº®¹¬´«ÈË")
		{
		   command("say ²»¹ı¼´ÒÑÈëÎÒÃÅÏÂ£¬ÕâÕÆÃÅÖ®Î»¾Í´«¸ø±ğÈË°Õ¡£\n");
		   rm("/data/zhangmen/zhangmen_chang_e.o");
		}
       		command("recruit " + ob->query("id") );
	       	ob->set("title",HBBLU+HIY"Å®æ´ÄïÄï×ùÏÂµÜ×Ó"NOR);
       		message("system", HIM"Å®æ´ÄïÄï"HIB"ÓÖĞÂÊÕÁËÒ»¸öµÜ×Ó "BLINK+HIR+this_player()->name()+NOR+HIW"
                     ÔÂ¹¬µÄÊÆÁ¦¼ÓÇ¿ÁË¡£\n"NOR,users());
