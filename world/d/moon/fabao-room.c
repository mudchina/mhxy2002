//By tianlin@mhxy for 2002.1.2ĞŞ¸Änuwa bossÈËÎïÈë¿Ú
//BY tianlin@mhxy for 2002.1.25,ĞŞ¸ÄÖı·¨±¦µôÄÚÁ¦enableµÄbug
#include <ansi.h>
#include <weapon.h>
#include <armor.h>
#include <login.h>
...
×Ö£º
                 ...
                 [1;37m[4;53mÀ¥ÂØ¾ø´¦[0m

                 [1;33m[4;53mÅ®æ´²¹Ìì[0m

                 [1;35m[4;53m²ÊÊ¯Î´¾¡[0m

                 [1;36m[4;53mÒÅ¸£ºóÈË[0m
...
list           ÁĞ¾Ù¿ÉÖÆ·¨±¦ÖÖÀà
upgrade        ·¨±¦Éı¼¶
cost ·¨±¦      ·¨±¦Á¶ÖÆ¼°Éı¼¶¿ÉÄÜ·ÑÓÃ
change_name    ¸ø·¨±¦¸ÄÃû
change_id      ¸ø·¨±¦¸Ä´úºÅ
change_desc    ¸ø·¨±¦¸ÄÃèÊö
change_unit    ¸ø·¨±¦¸Äµ¥Î»Ãû³Æ
Éı¼¶ÔİĞĞ°ì·¨£º
¡¡¡¡Ã¿Îå´ÎÉı¼¶¼ÓÒ»¸öĞÇ£¬×î¸ßÎªÎåĞÇ¡£
TEXT
	]));
        set("exits", 
        ([ //sizeof() == 1 
...
int do_sm()
{
   ...
   if( me->query("balance") < 500000 )
      return notify_fail("×ö·¨±¦·ÑÓÃÌ«¸ß£¬»¹ÊÇÏÈÈ¥·¢²Æ°É¡£\n");
...
{  
   ...
   //ÖÆÔìÌØÊâ·¨±¦
     ...
     {
          tell_object(ob,"Äã²»¹»ÖÆÔìÌõ¼ş£¬ÇëÓÃ cost ²éÑ¯¡£\n");
          return;
     }
     ob->set_temp("fabao_type","special");
     write("\n");
     write("¿ÉÑ¡ÔñÌØÊâ·¨±¦ÖÖÀà£º\n");
     write("1. ÌÓÅÜÀà  2. ÊÕÈ¡Àà  3. ¶¨ÉíÀà\n");
     write("ÇëÑ¡Ôñ£º(q ¼üÈ¡Ïû)");
   }   
...
//ÖÆÔìÌØÊâ·¨±¦ÕâÀïnewob1Ö»ÊÇÓÃÀ´´æ´¢Íæ¼Ò×Ô¼ºÉè¶¨µÄĞÅÏ¢
void build_special_fabao(object ob)
{
...
int  do_list()
{
   ...
   write(HIY"w."HIG" ÎäÆ÷£º\n"NOR);
   write("       ¼ÓÎïÀíÉËº¦Á¦(damage)\n");
   write("       ¼Ó·¨ÊõÉËº¦Á¦Ö®ÆøÑªÉËº¦Á¦(attack_qi)£¬\n"); //0-25
   write("       ¼Ó·¨ÊõÉËº¦Á¦Ö®¾«ÉñÉËº¦Á¦(attack_shen)¡£\n\n");//0-25   
   write(HIY"a."HIC" »¤¾ßÀà£º\n"NOR);
   write("       ¼Ó·ÀÓùÁ¦(dodge)\n");
   write("       ¼ÓµÖ¿¹Á¦(armor)\n");
   write("       ¼ÓÄÚ¹¦µÖ¿¹Á¦(armor_vs_force)\n");
   write("       ¼ÓÄ§·¨¹¥»÷Á¦(spells)\n");
   write("       ¼ÓÄ§·¨µÖ¿¹Á¦(armor_vs_spells)\n");
   write("       ¼Ó·¨Êõ·ÀÓùÁ¦Ö®ÆøÑª±£»¤Á¦(defense_qi)£¬\n");//0-25
   write("       ¼Ó·¨Êõ·ÀÓùÁ¦Ö®¾«Éñ±£»¤Á¦(defense_shen)£¬\n");//0-25
   write("       ¼Ó·¨Êõ·ÀÓùÁ¦Ö®·¨±¦ÊÕÈ¡Á¦(defense_shou)¡£\n\n");//0-25
   return 1;   
}
...
int do_cost(string arg)
{
   ...
   if( !arg || arg == "" )  {
      write("ÖÆÔì·¨±¦ĞèÒªÎåÊ®Á½»Æ½ğ¡£²¢ÇÒµÀĞĞ±ØĞëÔÚÎåÊ®ÄêÒÔÉÏ£¬·¨Á¦ĞŞÎªÖÁÉÙÄÜÌÚÔÆ¼İÎí£¬ÄÚÁ¦ĞŞÎªÖÁÉÙÈıÄêÒÔÉÏ·½¿É¡£ \n");
      write("ÌØÊâ·¨±¦²»ÄÜÉı¼¶¡£\n");
      return 1;
   }
   ...
   if( fabao_ob->query("series_no") == "1" )  
      ...
      for(i=0; i<damage_stars; i++)
         write("¡ï");
      write(NOR + "\n");
      if (damage_stars==5) write("ÄãµÄ·¨±¦"+HIG+"ÎïÀíÉËº¦Á¦"+NOR+"ÒÑ¾­²»ÄÜÔÙÉı¼¶ÁË¡£\n");
      else 
	  {
          write(HIG+"ÎïÀíÉËº¦Á¦" + NOR+ "(damage)Éı¼¶ĞèÒª£º\n");
          write("    µÀĞĞ"+chinese_number(100*M(damage_stars))+"ÄêÒÔÉÏ¡£\n");
          write("    ÏûºÄ½ÙÊı"+chinese_number(damage_stars)+"µã\n");
        }
		write("\n·¨ÊõÉËº¦Á¦£º\n");        
		write( fabao_ob->show_status() );
		
		if(a_qi >= 25 )
			write(RED"ÆøÑªÉËº¦Á¦" + NOR + "ÒÑĞŞÁ·µ½¶¥¡£\n");
		else
		{
			...
			write(RED"ÆøÑªÉËº¦Á¦" + NOR + "(attack_qi)Éı¼¶ĞèÒª£º\n");
			write("    µÀĞĞ"+chinese_number(dx_req/1000)+"Äê£¬ÄÚÁ¦ĞŞÎª"+chinese_number(neili_req/100)+"Äê¡£\n");
			write("    µÀĞĞ"+chinese_number(dx_cost/1000)+"Äê£¬ÄÚÁ¦ĞŞÎª"+chinese_number(neili_cost)+"µã(100µãÏàµ±ÓÚÒ»Äê)¡£\n");
		}

		if( a_shen >= 25 )
			write(BLU"¾«ÉñÉËº¦Á¦" + NOR + "ÒÑĞŞÁ·µ½¶¥¡£\n");
		else
		{
			...
			write(BLU"¾«ÉñÉËº¦Á¦" + NOR + "(attack_shen)Éı¼¶ĞèÒª£º\n");
			write("    µÀĞĞ"+chinese_number(dx_req/1000)+"Äê£¬·¨Á¦ĞŞÎª"+RANK_D->describe_fali(fali_req)+"¡£\n");
			write("    µÀĞĞ"+chinese_number(dx_cost/1000)+"Äê£¬·¨Á¦ĞŞÎª"+chinese_number(fali_cost)+"µã¡£\n");
		}        
      }
      ...
      if (dodge_stars==5) write("ÄãµÄ·¨±¦"+HIY+"·ÀÓùÁ¦"+NOR+"ÒÑ¾­²»ÄÜÔÙÉı¼¶ÁË¡£\n");
      else {
	  write(HIY+"·ÀÓùÁ¦" + NOR+ "(dodge)Éı¼¶ĞèÒª£º\n");
          write("    µÀĞĞ"+chinese_number(100*M(dodge_stars))+"ÄêÒÔÉÏ¡£\n");
          write("    ÏûºÄ½ÙÊı"+chinese_number(dodge_stars)+"µã\n");
          }

      if (armor_stars==5)  write("ÄãµÄ·¨±¦"+HIB+"µÖ¿¹Á¦"+NOR+"ÒÑ¾­²»ÄÜÔÙÉı¼¶ÁË¡£\n");
      else { 
	  write(HIB+"µÖ¿¹Á¦" + NOR+ "(armor)Éı¼¶ĞèÒª£º\n");
          write("    µÀĞĞ"+chinese_number(100*M(armor_stars))+"ÄêÒÔÉÏ¡£\n");
          write("    ÏûºÄ½ÙÊı"+chinese_number(armor_stars)+"µã\n");
          }
 
      if (force_stars==5) write("ÄãµÄ·¨±¦"+HIM+"ÄÚ¹¦µÖ¿¹Á¦"+NOR+"ÒÑ¾­²»ÄÜÔÙÉı¼¶ÁË¡£\n");
      else {
	  write(HIM+"ÄÚ¹¦µÖ¿¹Á¦" + NOR+ "(armor_vs_force)Éı¼¶ĞèÒª£º\n");
          write("    µÀĞĞ"+chinese_number(100*M(force_stars))+"ÄêÒÔÉÏ¡£\n");
          write("    ÏûºÄ½ÙÊı"+chinese_number(force_stars)+"µã\n");
	  }

      if (spells_stars==5) write("ÄãµÄ·¨±¦"+HIC+"Ä§·¨¹¥»÷Á¦"+NOR+"ÒÑ¾­²»ÄÜÔÙÉı¼¶ÁË¡£\n");
      else {
	  write(HIC+"Ä§·¨¹¥»÷Á¦" + NOR+ "(spells)Éı¼¶ĞèÒª£º\n");
          write("    µÀĞĞ"+chinese_number(100*M(spells_stars))+"ÄêÒÔÉÏ¡£\n");
          write("    ÏûºÄ½ÙÊı"+chinese_number(spells_stars)+"µã\n");
	  }

      if (vs_spells_stars==5) write("ÄãµÄ·¨±¦"+HIW+"Ä§·¨µÖ¿¹Á¦"+NOR+"ÒÑ¾­²»ÄÜÔÙÉı¼¶ÁË¡£\n");
      else {
	  write(HIW+"Ä§·¨µÖ¿¹Á¦"+NOR+ "(armor_vs_spells)Éı¼¶ĞèÒª£º\n");
          write("    µÀĞĞ"+chinese_number(100*M(vs_spells_stars))+"ÄêÒÔÉÏ¡£\n");
          write("    ÏûºÄ½ÙÊı"+chinese_number(vs_spells_stars)+"µã\n");
	  }
		write("\n·¨Êõ·ÀÓùÁ¦£º\n");  	
		write( fabao_ob->show_status() );
		
		if( d_qi >= 25 )
			write(RED"ÆøÑª±£»¤Á¦" + NOR + "ÒÑĞŞÁ·µ½¶¥¡£\n");
		else
		{
			...
			write(RED"ÆøÑª±£»¤Á¦" + NOR+ "(defense_qi)Éı¼¶ĞèÒª£º\n");
			write("    µÀĞĞ"+chinese_number(dx_req/1000)+"Äê£¬ÄÚÁ¦ĞŞÎª"+chinese_number(neili_req/100)+"Äê¡£\n");
			write("    µÀĞĞ"+chinese_number(dx_cost/1000)+"Äê£¬ÄÚÁ¦ĞŞÎª"+chinese_number(neili_cost)+"µã(100µãÏàµ±ÓÚÒ»Äê)¡£\n");
		}

		if( d_shen >= 25 )
			write(RED"¾«Éñ±£»¤Á¦" + NOR + "ÒÑĞŞÁ·µ½¶¥¡£\n");
		else
		{
			...
			write(BLU"¾«ÉñÉËº¦Á¦" + NOR+ "(defense_shen)Éı¼¶ĞèÒª£º\n");
			write("    µÀĞĞ"+chinese_number(dx_req/1000)+"Äê£¬·¨Á¦ĞŞÎª"+RANK_D->describe_fali(fali_req)+"¡£\n");
			write("    µÀĞĞ"+chinese_number(dx_cost/1000)+"Äê£¬·¨Á¦ĞŞÎª"+chinese_number(fali_cost)+"µã¡£\n");
		}

		if( d_shou >= 25 )
			write(YEL"·¨±¦ÊÕÈ¡Á¦" + NOR + "ÒÑĞŞÁ·µ½¶¥¡£\n");
		else
		{
			...
			write(YEL"·¨±¦ÊÕÈ¡Á¦" + NOR+ "(defense_shou)Éı¼¶ĞèÒª£º\n");
			write("    µÀĞĞ"+chinese_number(dx_req/1000)+"Äê£¬·¨Á¦ĞŞÎª"+RANK_D->describe_fali(fali_req)+"£¬ÄÚÁ¦ĞŞÎª"+chinese_number(neili_req/100)+"Äê¡£\n");
			write("    µÀĞĞ"+chinese_number(dx_cost/1000)+"Äê£¬·¨Á¦ĞŞÎª"+chinese_number(fali_cost)+"µã£¬ÄÚÁ¦ĞŞÎª"+chinese_number(neili_cost)+"µã(100µãÏàµ±ÓÚÒ»Äê)¡£\n");
		}	
   }   
   return 1;
}
...
int  do_upgrade(string arg)
{
   ...
   if( fabao_ob->query("equipped") )
      return notify_fail("Äã±ØĞë·ÅÏÂ·¨±¦²ÅÄÜĞŞÁ¶¡¢Éı¼¶¡£\n");
   ...
   if( fabao_ob->query("series_no") == "1")  
      {
		if( property == "attack_qi" )
		{
			...
			me->save();
			fabao_ob->save();
			message_vision(HIR "$N»º»º½«ÄÚÁ¦×¢Èë$n"HIR"ÖĞ£¬Ö»¼û$n"HIR"·Å³öÎå²ÊÏ¼¹â£¬»¶Ìø²»Ö¹£¡\n" NOR, me, fabao_ob);
			return 1;
		}
		...
		else if( property == "attack_shen" )
		{
			...
			me->save();
			fabao_ob->save();
			message_vision(HIR "$N»º»º½«·¨Á¦×¢Èë$n"HIR"ÖĞ£¬Ö»¼û$n"HIR"·Å³öÎå²ÊÏ¼¹â£¬»¶Ìø²»Ö¹£¡\n" NOR, me, fabao_ob);
			return 1;
		}
      		else if(property == "damage" )
      		{
      			stars = fabao_ob->query("stars/"+property);
      			if (stars==5) return notify_fail("ÄãµÄ·¨±¦ÒÑ¾­²»ÄÜÔÙÉı¼¶ÁË¡£\n");
      ...
      property != "defense_shen" && property != "defense_shou" )
         return notify_fail("·¨±¦Ã»ÓĞÕâ¸öĞŞÁ¶ÌØĞÔ£¬ÇëÓÃ cost <·¨±¦Ãû> À´²é¿´¡£\n");
