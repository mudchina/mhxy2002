...
void create()
{
        set_name(MAG"韦春芳"NOR, ({ HIR"wei chunfang"NOR,  "wei chunfang", "chunfang", "wei" }) );
        set("gender", "女性" );
        set("title", HIG"丽春院"HIW"老板娘"NOR);
//      set("nickname", "鹿鼎公");
        set("age", 42);
        set("long",
...
void greeting(object me)
{
    ...
    if (me->query("gender")=="无性" && file_name(environment(this_object()))=="/d/changan/lichunyuan") {
...
//		return notify_fail("自己卖自己？实在是发财有道了。\n");
	...
	command("rumor "HIY+ob->query("name")+HIM"被拐卖到丽春院了，不多不少，正好"HIW +
		CHINESE_D->chinese_number(ob->query("balance")) + NOR+YEL"个铜板哪。\n"NOR);
	...
	ob->set("title",HIY"立春院当红姑娘"NOR);
       ob->save();
	this_player()->add("balance",1800000);//给的固定的钱
	this_player()->set("title",HIR"拐卖妇女的人贩子"NOR);
       this_player()->save();
return 1;
}
