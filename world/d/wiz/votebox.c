...
//自动储存投票资料。。。(档案将会储存在/log/vote/ 目录下面)
...
void create()
{
        set_name( HIW"大话西游"+HIR"投票箱"NOR, ({ "vote box"}) );
        set_weight(9999999);
        set("unit", "个");
        set("no_clean_up",1);
        set("long",HIM"为了大话西游更好的发展，请投下您宝贵的一票，谢谢！"NOR);
        setup();
}
        ...
        if (arg=="." && vote_string!="")
         {
           ...
           if (this_object()->save())
              tell_object(me,"ok!设置投票内容完毕，同时已经储存在/log/vote/vote"+HIR+title+NOR".o\n");        
           else
...
int do_look(string arg)
{
        ...
        write(HIW"大话西游"HIR"投票箱"HIW",(请用choose 1|2|3..来投票,谢谢你支持大话西游的发展!)\n"NOR);
        write(HIG"=========================================================================\n"NOR);
        if (!ob->query("vote_string") || !ob->query("choose_number")) 
        {
                write(HIR"大话西游暂时没有投票活动!\n"NOR);
                return 1;
        }       
        write(HIY"本次投票内容\n"NOR);
...
int do_vote(string arg)
{
        ...
        ob->save();
        write_file("/log/vote/vote"+ob->query("vote_title")+".txt",me->query("id")+" vote "+i+"\n");
        write(HIM"本次投票,你选择了选项"+HIR+i+NOR+"\n");
        write(HIM"谢谢您对大话西游的衷心支持!GOOD LUCK!\n"NOR);
        return 1;
}       
