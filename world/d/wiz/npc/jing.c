...
set_name("°×¾§¾§",({"jing"}));
        ...
        set("age", 34);
        set("long",
	"ÕâÊÇ³¤°²³ÇÂé½«¹ÝµÄÅ®ÀÏ°å,Äã¿ÉÒÔ(ask jing about Âé½«)»ò(ask jing about play)¡£\n"
	"(ask jing about rule)»ò(ask jing about Íæ·¨)¡£\n");
        set("combat_exp", 100000);
        set("daoxing", 200000);
        set("title","Âé½«¹ÝÅ®ÀÏ°å");
        set("inquiry",([
           "mj": (: to_say_play :),
           "Âé½«": (: to_say_play :),
           "Íæ·¨": (: to_say_play :),
...
int do_setmj(string str)
{
        ...
        if (!str){
          ...
          if (who["AutoGon"]!="") numstr+="Óöµ½"+HIY+show_mj(who["AutoGon"],2)+NOR+"»á×Ô¶¯"+HIY+"[  ÅÆ]"+NOR+"¡£\n";
        ...
        if (str=="tin"){
           ...
           if (numstr=="none"){
              who["AutoGon"]="";
              return notify_fail("ÄãÈ¡Ïû×Ô¶¯  ÅÆÉè¶¨¡£\n");
           }
           check=do_check_Mj(who["Mj"],numstr);
           if (check==0) return notify_fail("ÄãÃ»"+do_check_num(numstr,0)+"ÕâÕÅÅÆ¡£\n");
           if (check < 3) return notify_fail("ÄãÊÖÖÐÒªÓÐÈþÕÅ"+do_check_num(numstr,0)+"²ÅÄÜÉè¶¨  ´ËÅÆ¡£\n");
           check=do_check_Mj(who["AutoGon"],numstr);
           if (check){
              who["AutoGon"]=do_delete_Mj(who["AutoGon"],numstr,1);
              return notify_fail("Äã"+HIR+"É¾µôÁË"+NOR+"Óöµ½"+do_check_num(numstr,0)+"»á×Ô¶¯  ÅÆµÄÉè¶¨¡£\n");
           }
           who["AutoGon"]+=numstr;
           Auto_Gon=1;
           write("ÄãÉè¶¨Óöµ½"+do_check_num(numstr,0)+"Ê±»á×Ô¶¯  ÅÆ¡£\n");
           return 1;
        }
        else if (str=="pon"){
           if (numstr=="none"){
