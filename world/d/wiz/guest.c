...
对不起,您的地址在梦幻西游的信誉被别人破坏了,为了保证
包括您在内的大多数玩家能够愉快地游戏，我们不得不限制
从您的网络ＩＰ地址的连线.请您在客厅稍候,梦幻西游巫师
...
{  
  ...
  if (!wizardp(who)&&userp(who))
  {
    ...
    MONITOR_D->report_system_object_msg (who,  
      "的IP被禁，现被引入梦幻西游迎客厅(/d/wiz/guest.c)。");  
    if (nb_guests(where) > 10)
    {
      tell_object (CYN"对不起，迎客厅里客人太多，请下次再来。\n"NOR,who);
...
{
  tell_object (who,"你可以使用approve来批准玩家进入梦幻西游世界。\n");
}
...
int do_approve (string arg)
{
  ...
  tell_object (me,"您批准"+who->query("name")+"进入梦幻西游世界！\n");
  tell_object (who,"欢迎您进入梦幻西游世界！\n");
