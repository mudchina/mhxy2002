...
string zm_apply();
int rank_me();
#define DIRNAME "/d/death/npc/"

#define MYFILE "/d/death/npc/zhangmen.c"

void create()
{
        set_name("大弟子", ({ "gui wang", "guiwang" }) );
        set("gender", "男性" );
        set("age", 30);
        set("str", 30);
        set("per", 30);
...
void init()
{
        ...
        me->create_identity("/d/death/npc/dizhang", "/d/death/new-walk2");
...
void reset_me (object me)
{
        ...
        me->set_name("大弟子", ({ "gui wang", "guiwang" }) );
        me->set("gender", "男性" );
        me->set("title",HIB"大力鬼王"NOR);
...
int convert_identity (object me, object ob)
{
		...
		ob->set("zhangmen/title", HIB"大力鬼王"NOR);
        	ob->set("title",HIB"大力鬼王"NOR);
        } else {	
		ob->set("zhangmen/title", HIM"地府女主"NOR);
        	ob->set("title",HIM"地府女主"NOR);
        }	
        ob->save();
        ...
        if (ob->query("gender")=="男性") {
        	me->set_name(ob->query("name"), ({ "gui wang", "guiwang" }) );
	       	me->set("title",HIB"大力鬼王"NOR);
        } else{
        	me->set_name(ob->query("name"), ({ "nu zhu", "nu zhu" }) );
	       	me->set("title",HIM"地府女主"NOR);
	}       	
...
"你曾叛师欺祖，言无信行不轨，岂能出任掌门弟子一职！\n";
        ...
        ob->set_temp("death_zm_applied", 1);
        if( (string)ob->query("gender") == "男性" )
                return
"好！师兄有意出任大力鬼王一职？我们就切磋一下吧！\n";
        else return "好！师姊有意出任地府女主一职？我们就切磋一下吧！\n";
}
...
int accept_fight(object ob)
{
        ...
        {
                command ("say 失陪了！\n");
                return notify_fail("这位"+ str+"尚在比武之中，请稍候。\n");
        }

        if (ob->query_temp("death_zm_applied"))
        {
                command("say 好！尚请未来的"+ str+"手下留情。\n");
        }
        else
        {
...
ob->query_temp("death_zm_applied"))
                {
                        message_vision
("$N皱了皱眉道：此次比武我未能静心尽力，希望重新来过。\n", me);
                ...
                if( ob->query_temp("death_zm_applied") != 1 )
                        return 1;
...
void master_announce (object me, object who, object ob)
{
	...
	message("shout", HIW"〖"HIB"冥界鬼王"HIW"〗"HIM"地藏王菩萨(Dizang pusa)："HIY"今日是本门"NOR+HIB"大力鬼王"NOR+YEL+this_player()->name()+HIY"上任的日子，恭请各位仙长多加捧场！\n"NOR,users());
        ob->delete_temp("death_zm_applied");
        me->set("new_player",1);
...
int rank_me()
{
	...
	exp=(int)me->query("combat_exp");
        if( me->query("title") == HIM"地府女主"NOR
          ||me->quert("title") == HIB"大力鬼王"NOR)
		return notify_fail("你已经是掌门身份，还来要什么职位．\n");

        if( me->query("title") == HIB"阴曹地府"NOR+HBBLU+HIR"冥王座下弟子"NOR)
		return notify_fail("你已经是冥王座下弟子了，还来要什么职位．\n");

	if( me->query("family/family_name") == "阎罗地府")
	{
                if( exp <= 20000 ){
			me->set("title", HIB"阎罗地府"NOR+YEL"游魂"NOR);
		} else if(exp <= 50000 ){
			me->set("title", HIB"阎罗地府"NOR+CYN"小鬼卒"NOR);
		} else if(exp < 100000 ){
			me->set("title", HIB"阎罗地府"NOR+HIR"催命鬼卒"NOR);
		} else if(exp <= 200000 ){
			me->set("title", HIB"阎罗地府"NOR+HIY"小无常"NOR);
		} else if(exp <= 500000 ){
			me->set("title", HIB"阎罗地府"NOR+HIC"帐前文案"NOR);
		} else if(exp <= 1000000 ){
			me->set("title", HIB"阎罗地府"NOR+HIR"无情判官"NOR);
		} else if(exp <= 2000000 ){
			me->set("title", HIR"十八地狱"HIY"按查使"NOR);
		} else {
			me->set("title", HIB"阎罗地府"NOR+HIW"鬼族总领"NOR);
		}
