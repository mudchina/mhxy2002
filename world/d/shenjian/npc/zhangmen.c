...
string zname(object);
int rank_me();
#define DIRNAME "/d/shenjian/npc/"

#define MYFILE "/d/shenjian/npc/zhangmen.c"
...
void init()
{
	...
	me->create_identity("/d/shenjian/npc/zhen", "/d/shenjian/sjsz");
...
{
	message("shout", HIR"¡¾"HIW"Éñ½£É½×¯"HIR"¡¿"HIC"ÌúÕæÕæ(Tie zhenzhen)£º"HIY"½ñÈÕÊÇ±¾ÃÅ"NOR+HIC"¶áÃü½£ÏÀ"NOR+YEL+this_player()->name()+HIY"ÉÏÈÎµÄÈÕ×Ó£¬»¹ÇëÖîÎ»ÉñÏÉ¶à¶àÅõ³¡¡£\n"NOR,users());
...
string zname(object ob)
{
	if( (string)ob->query("gender") == "Å®ÐÔ" ) 
		return HIC"·ÉÌìÅùö¨"HIY"Å®ÏÀ"NOR;
	else return HIR"É±ÈË²»µÎÑª"NOR+HIC"½£ÏÀ"NOR;
}
int rank_me()
{
	...
	exp=exp+dx;
        if( me->query("title") == "Éñ½£É½×¯"HIC"·ÉÌìÅùö¨"HIY"Å®ÏÀ"NOR
          ||me->query("title") == "Éñ½£É½×¯"HIR"É±ÈË²»µÎÑª"NOR+HIC"½£ÏÀ"NOR)
		return notify_fail(BLINK+HIR"ÄãÒÑ¾­ÊÇÕÆÃÅÉí·Ý£¬»¹À´ÒªÊ²Ã´Ö°Î»£®\n"NOR);
	if( me->query("family/family_name") == "Éñ½£É½×¯")
	{
                if( exp <= 40000 ){
			me->set("title", HBBLU+HIW"Éñ½£É½×¯³ø·¿×Ü¹Ü"NOR);
		} else if(exp <= 150000 ){
			me->set("title", HBBLU+HIW"Éñ½£É½×¯¿ªÂ·ÏÈ·æ"NOR);
		} else if(exp < 500000 ){
			me->set("title", HBBLU+HIW"Éñ½£É½×¯ÊØÎÀ"NOR);
		} else if(exp <= 800000 ){
		       if(me->query("gender") == "ÄÐÐÔ"){	
			me->set("title", HBBLU+HIW"Éñ½£É½×¯"NOR+YEL"Ð¡½£ÏÀ"NOR);
		       }
		       else me->set("title",HBBLU+HIW"Éñ½£É½×¯"NOR+HIM"Å®½£ÏÀ"NOR);
		} else if(exp <= 1500000 ){
		       if(me->query("gender") == "ÄÐÐÔ"){
			   me->set("title", HBBLU+HIW"Éñ½£É½×¯"NOR+MAG"ÓñÊ÷ÁÙ·ç"HIC"½£¿Í"NOR);
		       }
		       else me->set("title",HBBLU+HIW"Éñ½£É½×¯"NOR+HIM"Óñµñ±ùËÜ"HIC"Å®ÏÀ¿Í"NOR);
		} else if(exp <= 3000000 ){
			me->set("title", HBBLU+HIW"Éñ½£É½×¯"NOR+HIR"µ¶½£ÎÞµÐÏÀ"NOR);
		} else if(exp <= 5000000 ){
			me->set("title", HBBLU+HIW"Éñ½£É½×¯"NOR+HBRED+HIC+U" Òþ     Ê¿ "NOR);
		} else {
			me->set("title", HBBLU+HIW"Éñ½£É½×¯"NOR+HIC"Ë£½£"HIY"ÈçÉñ"HIG"½£ÏÀ"NOR);
		}
