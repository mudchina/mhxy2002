...
void create()
{
 set_name(HIR"重云"+HIW"棍"NOR, ({ HIR"chongyun bang"NOR, "bang", "stick" }) );
        set_weight(4000);
        if( clonep() )
...
int ji_ob(object victim)
{
        ...
        fabao->set("spi", 0);
         fabao->add("spi", 6000);
        if( me->is_busy() )
   return notify_fail("你正忙着呢，无法祭出"+HIR"重云"+HIW"棍。\n"NOR);
        if( !victim )
                return notify_fail("你想祭的人不在这里。\n");
        if( where->query("no_magic") )
        return notify_fail("这里不能祭"+HIR"重云"+HIW"棍。\n"NOR);
        if( !me->is_fighting() )
                return notify_fail("只有战斗中才能祭出。\n");
        if( (int)me->query("mana") < 800 )
     return notify_fail("你的法力不能控制"+HIR"重云"+HIW"棍。\n"NOR);

message_vision(YEL"\n$N大喝一声“去!”，只见$n"+YEL"「呼」地一下化做一条巨龙飞到半空。\n"NOR, me, fabao);

message_vision(YEL"霎那间天色一变！$n"+YEL"呼啸着向"+HIW"$N"+YEL"迎面砸下！\n"NOR, victim, fabao);
...
message_vision(WHT"\n$N手一指大喝一声“来！”，只听$n"+WHT"发出一声轻哮！飞到了"+HIR"$N"+HIW"手中。\n"NOR, victim, fabao);
                ...
                else
                {
                     ...
                     if( damage > 3000 ) damage = 3000;
                        ...
                        if( damage > 0 )
                        {
message_vision(HIY"\n结果$N"+HIY"来不及闪开，白光闪过之后，$n"+HIY"已经砸在"+HIW"$N"+HIY"的头上,\n"+HIY"        鲜血 扑 的喷出来！\n\n"+"        $n"+HIW"通过这次实践,攻击力加强了.\n"NOR, victim, fabao,victim);
