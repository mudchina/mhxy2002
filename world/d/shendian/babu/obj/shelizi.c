...
int do_mo(string arg)
{
    ...
    if (!arg)
 return notify_fail("你想让舍利子变成什么兵器？(sword blade stick hammer fork axe spear staff whip pa mace)\n");
        else
        {
                sscanf(arg,"%s",type);
                        switch(type)
...
message("channel:rumor",YEL+"\n【"+HIB" 八部天龙 "NOR+YEL"】某人："+who->query("name")+"拿到"+HIR" 凝绝"+HIW"鞭 "NOR+YEL"了！\n\n"+NOR,users() );
        ...
        destruct(this_object());
                                break;
//by tianlin 2001.8.3.加入pa,为了加开封府这个门派
                       case "pa":
message_vision(WHT"\n$N轻轻的摸了一下"NOR+HIR"舍利子"NOR+WHT"，只见白光一闪，"+HIR" 舍利子 "NOR+WHT"不由自主的散开，形成一把"NOR+HIR" 七星"NOR+HIW"耙 \n"NOR,who);
message("channel:rumor",YEL+"\n【"+HIB" 八部天龙 "NOR+YEL"】某人："+who->query("name")+"拿到"+HIR" 七星"+HIW"耙 "NOR+YEL"了！\n\n"+NOR,users() );
        ob = new("/d/shendian/babu/obj/gupa");
         ob->move(who);
        destruct(this_object());
                                break;
//by tianlin@mhxy for 2001.10.3.加入mace,
                       case "mace":
message_vision(WHT"\n$N轻轻的摸了一下"NOR+HIR"舍利子"NOR+WHT"，只见白光一闪，"+HIR" 舍利子 "NOR+WHT"不由自主的散开，形成一把"NOR+HIR" 风雷"NOR+HIW"锏 \n"NOR,who);
message("channel:rumor",YEL+"\n【"+HIB" 八部天龙 "NOR+YEL"】某人："+who->query("name")+"拿到"+HIR" 风雷"+HIW"锏 "NOR+YEL"了！\n\n"+NOR,users() );
