...
void create()
{
 set_name(HIR"吟龙"+HIW"锤"NOR, ({ HIR"yinlong chui"NOR, "chui", "hammer" }) );
        set_weight(4000);
        if( clonep() )
...
int ji_ob(object victim)
{
        ...
        fabao->set("spi", 0);
         fabao->add("spi", 6000);
        if( me->is_busy() )
   return notify_fail("你正忙着呢，无法祭出"+HIR"吟龙"+HIW"锤。\n"NOR);
        if( !victim )
                return notify_fail("你想祭的人不在这里。\n");
        if( where->query("no_magic") )
      return notify_fail("这里不能祭"+HIR"吟龙"+HIW"锤。\n"NOR);
        if( !me->is_fighting() )
                return notify_fail("只有战斗中才能祭出。\n");
        if( (int)me->query("mana") < 800 )
     return notify_fail("你的法力不能控制"+HIR"吟龙"+HIW"锤。\n"NOR);
