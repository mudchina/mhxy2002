...
int rank_me()
{
        ...
        if( me->query("title") == "´óÑ©É½ÕÆÃÅ")
		return notify_fail("ÄãÒÑ¾­ÊÇÕÆÃÅÈËÁË£¬»¹À´ÒªÊ²Ã´Ö°Î»£®\n");
        if( (string)me->query("family/family_name") == "´óÑ©É½"){
                if( exp <= 100000 ){
                        me->set("title", HIR"Î÷Óò´óÑ©É½"HIW"Ñ²É½Ğ¡Ä§"NOR);
                  } else if(exp < 1000000 ){
                        me->set("title", HIR"Î÷Óò´óÑ©É½"HIW"±ù¹ÈÑ²Ê¹"NOR);
                  } else if(exp < 2000000 ){
                        me->set("title", HIR"Î÷Óò´óÑ©É½"HIW"»¤ÅÉ×ğÕß"NOR);
                  } else if(exp < 3000000 ){
                        me->set("title", HIR"Î÷Óò´óÑ©É½"HIW"´óÅô½«¾ü"NOR);
                } else {
                        me->set("title", HIR"Î÷Óò´óÑ©É½"HIW"¾ÅÒõº®Ä§"NOR);
                }

                tell_object(me, "ğĞğÄ½«¾ü¶ÔÄãËµ£ºÄã¿ÉÒÔ×ö"+me->query("title")+"¡£\n");
                tell_room(environment(me),
"ğĞğÄ½«¾ü¶Ô"+me->query("name")+"Ëµ£ºÄã¿ÉÒÔ×ö"+me->query("title")+"¡£\n", ({me, me}));
...
varargs void drool(string msg, string who)
{
	...
	||	(strsrch(msg, "Drooler") >= 0)) {
		if( sscanf(msg, "%*s?%s", msg) == 2 ) msg = "Äã" + msg;
		...
		if( sscanf(msg, "%*s?%s", msg) == 2 ) msg = "ÎÒ" + msg;
