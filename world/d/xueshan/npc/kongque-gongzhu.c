...
LONG );
	...
	set_skill("force", 110);   
	set_skill("ningxie-force", 110);
	set_skill("literate", 90);
	set_skill("spells", 100);
	set_skill("dengxian-dafa", 110);
    ...
    carry_object("/d/xueshan/obj/fenghuangqin")->wield();
}
...
void attempt_apprentice(object ob)
{
	...
	if( (string)ob->query("family/family_name")=="´óÑ©É½" ){
		if( (int)ob->query("family/generation") < 2  ){
			...
			message("system", HIB"ÎÒ´óÑ©É½ÓÖ¶àÁËÒ»¸öµÜ×Ó  "HIR+this_player()->name()+HIW"
                 ´óÑ©É½"HIY"µÄÊÆÁ¦¼ÓÇ¿ÁË¡£\n"NOR,users());
		}
		else {
