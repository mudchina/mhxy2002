...
LONG);
        ...
        ([ //sizeof() == 4
                ...
                "southwest" : __DIR__"hmg3",
                "northwest" : __DIR__"hmg7",
                "northeast" : __DIR__"hmg9",
]));
        setup();
}
...
void init()
{
        ...
        me->add_temp("hmg5",1);
        if( me->query_temp("hmg_ready_out")==1 && me->query_temp("hmg5")<3 )
        call_out("send_out", 5, me);
        if( me->query_temp("hmg5")>=3 )
        {
                reset_path(me);
...
{
        if( me->query_temp("hmg5")>=3 || me->query_temp("hmg_ready_out") )
