...
#define DIRNAME "/d/mojiao/npc/"

#define MYFILE "/d/mojiao/npc/zhangmen.c"
...
void init()
{
   ...
   me->create_identity("/d/mojiao/npc/master", "/d/mojiao/gate");
...
string zm_apply()
{
   ...
   ob->set_temp("mojiao_zm_applied", 1);
...
int accept_fight(object ob)
{
   ...
   {
     command ("say 失陪了！\n");
     return notify_fail("这位大师兄尚在比武之中，请稍候。\n");
   }

   if (ob->query_temp("mojiao_zm_applied"))
   {
...
int check_result(object me, object ob)
{
   ...
   if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 )
   {
     ...
     if (me->query_temp("zhangmen/kill") && ob->query_temp("mojiao_zm_applied"))
   {
        message_vision ("$N皱了皱眉道：此次比武我未能静心尽力，希望重新来过。\n", me);
        return 1;
    }

     if( ob->query_temp("mojiao_zm_applied") != 1 )
        return 1;   
...
{
   CHANNEL_D->do_channel(who,"chat","本教掌门弟子"+ob->query("name")+"今日走马上任。恭请各位仙长多加捧场！");
   ob->delete_temp("mojiao_zm_applied");
   me->set("new_player",1);
...
int rank_me()
{
	...
	exp=exp+dx;
        if( me->query("title") == "魔教"NOR+HIC"邪天公主"NOR
          ||me->query("title") == "魔教"NOR+HIR"镇教明王"NOR)
		return notify_fail(BLINK+HIR"你已经是教主身份，还来要什么职位．\n"NOR);
	if( me->query("family/family_name") == "魔教")
	{
                if( exp <= 40000 ){
			me->set("title", BLU"魔教小魔妖"NOR);
		} else if(exp <= 1500000 ){
			me->set("title", HIB"魔教巡逻妖"NOR);
		} else if(exp < 500000 ){
			me->set("title", HIY"魔教教众"NOR);
		} else if(exp <= 800000 ){
		       if(me->query("gender") == "男性"){	
			me->set("title", HIR"魔教"NOR+HIY"神威长老"NOR);
		       }
		       else me->set("title",HIR"魔教"NOR+HIM"执法魔女"NOR);
		} else if(exp <= 1500000 ){
		       if(me->query("gender") == "男性"){
			   me->set("title", HIR"魔教"NOR+HIG"教主左护法"NOR);
		       }
		       else me->set("title",HIR"魔教"NOR+HIM"教主右护法"NOR);
		} else if(exp <= 3000000 ){
			me->set("title", HIR"魔教"NOR+WHT"法王"NOR);
		} else if(exp <= 5000000 ){
			me->set("title", HIR"魔教"NOR+HIB"副"HIC"  教 主"NOR);
		} else {
			me->set("title", HIR"魔教"NOR+HIC"教  主"NOR);
		}
