// 拍卖师
//BY tianlin@mhxy for 2001.12.30修改了一下颜色
inherit NPC;
#include <ansi.h>;
...
int do_paimai(string arg)
{
	...
	command("sys "+who->query("name")+"("+who->query("id")+")"+NOR+ob->query("name")+HIG+"进行拍卖。\n"NOR);
...
int begin_paimai(string name,int money,int num,int m)
{
	...
	set("on_paimai",1);
	mon = to_ch(money);
	command("bill "+NOR+name+HIC+"底价"+mon+"第"BLINK HIR""+chinese_number(m)+""NOR HIC"次"NOR);
	command("bill "+name+HIC"是本店第『"+chinese_number(num)+"』号拍卖品，有意者请使用CHU命令进行竞买。\n"NOR);
...
int finish_paimai(int num,int sign)
{
	...
	if (sign==0)
	{
		command("bill "+query("npaimai/" + sprintf("%d", num))+HIC+" 的所有权拍卖流标了。"+NOR);
		if(owner)
	...
	{
		command("bill  成交!"+buyer->query("id")+"获得了 "+NOR+query("npaimai/" + sprintf("%d", num))+HIG+" 的所有权。"+NOR);
	}
	else
	{
		command("bill 成交! 有人获得了 "+NOR+query("npaimai/" + sprintf("%d", num))+HIC+" 的所有权。"+NOR);
		command("sys  成交!"+buyer->query("id")+"获得了 "+NOR+query("npaimai/" + sprintf("%d", num))+HIG+" 的所有权。"+NOR);
	}
		if (buyer)
...
int report_paimai(object buyer,int money,int num)
{
	...
	if(!buyer->query_temp("invisibility_buyer"))
	{
	   if (!query("pnum"))
		   add("pnum",1);
		command("bill  "+buyer->query("id")+" 出价 "+mon+"购入"+NOR+query("npaimai/" + sprintf("%d", num))+HIC"的所有权。");
	...
	else
	{
	   if (!query("pnum"))
    	   add("pnum",1);
       command("sys  "+buyer->query("id")+" 出价 "+mon+"购入"+NOR+query("npaimai/" + sprintf("%d", num))+HIG"的所有权。");
       command("bill  有人出价 "+chinese_number(money)+"两银子 购入"+NOR+query("npaimai/" + sprintf("%d", num))+HIC"的所有权。");
...
int go_paimai(string id,string name,int money,int num,int m)
{
	...
	m+=1;
	mon = to_ch(money);
	command("bill "+id+" 出价 "+mon+"购入 "+NOR+name+HIC+" 的所有权，第"BLINK HIR""+chinese_number(m)+""NOR HIC"次"NOR);// by tianlin 2001.7.1
	command("bill "+name+HIC"是本店第『"+chinese_number(num)+"』号拍卖品，有意者请使用CHU命令进行竞买。\n"NOR);
	if (!(m==3))
