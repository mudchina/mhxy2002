...
void create()
{
  set_name(HIW"新手礼物台" NOR ,({"gift table","table"}));
  set("long","玩家可以在这里拿(pick)礼物(gift)。不是新手不要拿，否则后果自负。");
  set("unit", "个");
...
int get_gift(string arg)
{
  ...
  if (me->query("young_gift")) return notify_fail("你已经拿过礼物了,别太贪心哟！\n"); //"标记"限制
  message_vision("$N从礼物台上拿下一份礼物。\n" NOR, me); //拿下礼物时的动作描述
  lw=new("d/city/obj/gift"); //礼物路径    
  lw->move(me); //礼物所给的对象看前面的me的指代
  me->set("young_gift",1); //做标记
  return 1;
}
