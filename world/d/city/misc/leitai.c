...
void create ()
{
  restore();
  set ("short", "ÀÞÌ¨");
  set ("long", "\n\t       [44m[1;33m°Ë  ÏÉ  ¹ý  º£ [2;37;0m        [44m[1;32m¸÷  ÏÔ  Éñ  Í¨[2;37;0m\n\n");
  set("exits",([]));
  set("broadcast",1);
...
//    string *banned_cast=({"escape","chuqiao","qiankun","jieti","tudun","shuidun","yinshen"});
    ...
    write("ÀÞÌ¨ÉÏÇëÎðÊ¹ÓÃ"+arg+",¶àÐ»ºÏ×÷¡£\n");
    return 1;
}
...
void finish(int r) {
  ...
  if (!r) {     
    message_vision(HIG"$N$n´óÕ½Èý°Ù»ØºÏ£¬²»·ÖÊ¤°Ü£¬¸÷×ÔÀë³¡£¬¸ÄÈÕÔÙÕ½£¡\n"NOR,winner,loser);
    loser->move(__DIR__"kantai");
  } else {
     message_vision(HIG"$N°Ñ$n´ò·­ÔÚµØ£¬·ÉÆðÒ»½Å£¬½«$nÌßÏÂÁËÀÞÌ¨¡£\n"NOR,winner,loser);
     loser->move(__DIR__"kantai");
     message_vision(HIG"$NÔÚ¹ÛÖÚµÄ»¶ºôÉùÖÐ£¬¸ß¸ßÐËÐËµØÏÂ³¡ÐÝÏ¢ÁË¡£\n"NOR,winner);
...
void start() {
  ...
  object p2=query("player2");
  if ((!p1) || (!p2)) return;
  message_vision(HIW"$NºÍ$n±ÈÎä¿ªÊ¼£¡ÏÞÊ±Îå·ÖÖÓ£¡\n"NOR,p1,p2);  
  delete("no_fight");
  delete("no_magic");
  p1->kill_ob(p2);
  p2->kill_ob(p1);
