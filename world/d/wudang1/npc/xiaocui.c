...
void create()
{
	...
	set("attitude", "friendly");
        ...
        set("combat_exp", 1000+random(8000));
        set("daoxing", 10000+random(8000));
        set("tea_count", 5);
        ...
        setup();
        carry_object("/d/wudang1/obj/greenrobe")->wear();
...
int accept_object(object who, object ob)
{
		...
		|| (string)ob->query("name") == "金项链"  ) 
	{
		if ( (string)ob->query("name") == "金项链" ) 
		{
			...
			command("wear lace");
//			carry_object("/d/wudang1/obj/necklace")->wear();
		}
...
void serve_tea(object who)
{
	...
	{
	 	obn = new("d/wudang1/obj/dawancha");
		obn->move(room);
	...
	} else 
	{
		who->add_temp("tea_cup", -1);
	 	obn = new("d/wudang1/obj/xiangcha");
		add("tea_count",-1);
		obn->move(room);
