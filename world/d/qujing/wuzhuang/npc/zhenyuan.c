...
//By tianlin@mhxy for 2002.3.2优化
#define DEBUG 0
...
set("title", HIM"五庄观"HIY"观主"NOR);
set("nickname", HIC"大罗真仙"NOR);
   set("gender", "男性" );
   set("age", 65);
   set("per", 24);
   set("str", 30);
...
if(me->query("secmieyao")&&!me->query("m_mieyao"))
{
      me->add("daoxing",-2000);
      me->set("m_mieyao",1);
...
int rank_me()
{
        ...
        if( me->query("title") == "五庄观掌门")
		return notify_fail("你已经是掌门人了，还来要什么职位．\n");
        if( (string)me->query("family/family_name") == "五庄观"){
                if( exp <= 100000 ){
                        me->set("title", CYN"万寿五庄观"BLU"清风道童"NOR);
                } else if(exp <= 1000000 ){
                        me->set("title", CYN"万寿五庄观"BLU"八卦道长"NOR);
                } else if(exp < 2000000 ){
                        me->set("title", CYN"万寿五庄观"BLU"三界散仙"NOR);
                } else if(exp <= 3000000 ){
                        me->set("title", CYN"万寿五庄观"BLU"闲云野鹤大仙"NOR);
                } else {
                        me->set("title", CYN"万寿五庄观"BLU"地仙之祖"NOR);
                }

                tell_object(me, "镇元大仙对你说：你可以做"+me->query("title")+"。\n");
                tell_room(environment(me),
"镇元大仙对"+me->query("name")+"说：你可以做"+me->query("title")+"。\n", ({me, me}));
