...
string zname(object);
#include <ansi.h>
#define DIRNAME "/d/qujing/wuzhuang/npc/"

#define MYFILE "/d/qujing/wuzhuang/npc/zhangmen.c"
...
void init()
{
	...
	me->create_identity("/d/qujing/wuzhuang/npc/zhenyuan", "/d/qujing/wuzhuang/jiangjing");
...
int accept_fight(object ob)
{
	...
	if (ob->query_temp("wzg_zm_applied"))
	{
...
int check_result(object me, object ob)
{
	...
	if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 )
	{
		...
		if( ob->query_temp("wzg_zm_applied") != 1 )
			return 1;	
...
{
	message("shout", HIC"〖"HIW"大罗真仙"HIC"〗镇元大仙(Zhenyuan daxian)："HIY"今日是本门"NOR+HIC"大罗真仙"NOR+YEL+this_player()->name()+HIY"上任的日子，还请诸位神仙多多捧场。\n"NOR,users());
...
int rank_me()
{
	...
	exp=(int)me->query("combat_exp");
        if( me->query("title") == HIC"大罗真仙"NOR)
	...
	{
                if( exp <= 20000 ){
			me->set("title", HIB"五庄观护观仙童"NOR);
		} else if(exp <= 150000 ){
			me->set("title", HIB"五庄观守园力士"NOR);
		} else if(exp < 500000 ){
			me->set("title", RED"五庄观伏魔力士"NOR);
		} else if(exp <= 800000 ){
			me->set("title", YEL"五庄观松月真人"NOR);
		} else if(exp <= 1500000 ){
			me->set("title", WHT"五庄观驾鹤使者"NOR);
		} else if(exp <= 3000000 ){
			me->set("title", HIB"五庄观"NOR+YEL"南山不老仙"NOR);
		} else if(exp <= 5000000 ){
			me->set("title", HIB"五庄观"HIY"八卦真仙"NOR);
		} else {
			me->set("title", HIB"五庄观"HIR"真武道尊"NOR);
		}
...
HIR""+ob->query("name")+"对"+me->query("name")+"说：你可以做"+me->query("title")+"。\n"NOR, ({me, me}));
	...
	{
                if( exp <= 20000 ){
			me->set("title", "五庄观入门女徒");
		} else if(exp <= 50000 ){
			me->set("title", "五庄观琴童");
		} else if(exp < 100000 ){
			me->set("title", "方寸山内院主管");
		} else if(exp <= 200000 ){
			me->set("title", "五庄观驾鹤使者");
		} else if(exp <= 500000 ){
			me->set("title", "五庄观凝香仙姑");
		} else if(exp <= 1000000 ){
			me->set("title", "五庄观花容圣姑");
		} else if(exp <= 2000000 ){
			me->set("title", "方寸山怜星圣母");
		} else {
			me->set("title", "五庄观邀月天仙");
		}
