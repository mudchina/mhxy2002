...
string zname(object);
#include <ansi.h>
#define DIRNAME "/d/qujing/kusong/npc/"

#define MYFILE "/d/qujing/kusong/npc/zhangmen.c"

void create()
{
	set_name("总洞主", ({ "zhang men", "zhangmen" }) );
	set("gender", "男性" );
	set("age", 30);
	set("str", 30);
	set("per", 30);
...
void init()
{
	...
	me->create_identity("/d/qujing/kusong/npc/honghaier", "/d/qujing/kusong/guaishiya");
...
string zm_apply()
{
	...
	ob->set_temp("kusong_zm_applied", 1);
	return "好！\n";
}
...
int accept_fight(object ob)
{
	...
	if (ob->query_temp("kusong_zm_applied"))
	{
...
int check_result(object me, object ob)
{
	...
	if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 )
	{
		...
		if (me->query_temp("zhangmen/kill") && ob->query_temp("kusong_zm_applied"))
		{
			message_vision ("$N皱了皱眉道：此次比武我未能静心尽力，希望重新来过。\n", me);
			return 1;
		}

		if( ob->query_temp("kusong_zm_applied") != 1 )
			return 1;	
...
{
	message("shout", HIC"〖"RED"善财童子"HIC"〗红孩儿(Honghaier)："HIY"今日是本门"NOR+RED"善财童子"NOR+YEL+this_player()->name()+HIY"上任的日子，还请诸位神仙多多捧场。\n"NOR,users());
	ob->delete_temp("kusong_zm_applied");
	me->set("new_player",1);
...
int rank_me()
{
        ...
        if( me->query("title") == "善财童子")
		return notify_fail("你已经是善财童子了，还来要什么职位．\n");
        if( (string)me->query("family/family_name") == "火云洞")
        {
                if( exp <= 10000 ){
                        me->set("title", RED"枯松涧火云洞小妖"NOR);
                } else if(exp <= 150000 ){
                        me->set("title", RED"枯松涧火云洞先锋"NOR);
                } else if(exp < 500000 ){
                        me->set("title", RED"枯松涧火云洞健将"NOR);
                } else if(exp <= 800000 ){
                        me->set("title", RED"枯松涧火云洞巡山大将"NOR);
                } else if(exp <= 1500000 ){
                        me->set("title", RED"枯松涧火云洞妖王"NOR);
                } else if(exp <= 3000000 ){
                        me->set("title", RED"枯松涧火云洞魔王"NOR);
                } else if(exp <= 5000000 ){
                        me->set("title", RED"枯松涧火云洞长老"NOR);
                } else {
                        me->set("title", RED"枯松涧火云洞洞主"NOR);
                }
