...
string zname(object);
#include <ansi.h>
#define DIRNAME "/d/qujing/wudidong/npc/"

#define MYFILE "/d/qujing/wudidong/npc/zhangmen.c"
...
void init()
{
	...
	me->create_identity("/d/qujing/wudidong/npc/yushu", "/d/qujing/wudidong/houdong");
...
string zm_apply()
{
	...
	ob->set_temp("wudidong_zm_applied", 1);
	return "好！\n";
}
...
int accept_fight(object ob)
{
	...
	if (ob->query_temp("wudidong_zm_applied"))
	{
...
int check_result(object me, object ob)
{
	...
	if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 )
	{
		...
		if (me->query_temp("zhangmen/kill") && ob->query_temp("wudidong_zm_applied"))
		{
			message_vision ("$N皱了皱眉道：此次比武我未能静心尽力，希望重新来过。\n", me);
			return 1;
		}

		if( ob->query_temp("wudidong_zm_applied") != 1 )
			return 1;	
...
{
	message("shout", HIC"〖"CYN"食天鼠王"HIC"〗玉鼠精(Yu shu)："HIY"今日是本门"NOR+WHT"食天"NOR+CYN"鼠王"NOR+YEL+this_player()->name()+HIY"上任的日子，还请诸位神仙多多捧场。\n"NOR,users());
	ob->delete_temp("wudidong_zm_applied");
	me->set("new_player",1);
...
int rank_me()
{
	...
	exp=(int)me->query("combat_exp");
        if( me->query("title") == WHT"食天"NOR+CYN"鼠王"NOR)
		return notify_fail("你已经是掌门身份，还来要什么职位．\n");
	if( me->query("family/family_name") == "陷空山无底洞")
	{
                if( exp <= 20000 ){
			me->set("title", CYN"陷空山无底洞守门小妖"NOR);
		} else if(exp <= 150000 ){
			me->set("title", CYN"陷空山无底洞帮厨小妖"NOR);
		} else if(exp < 500000 ){
			me->set("title", CYN"陷空山无底洞万骨皆枯老魔"NOR);
		} else if(exp <= 800000 ){
			me->set("title", CYN"陷空山无底洞万骨皆枯老魔"NOR);
		} else if(exp <= 1500000 ){
			me->set("title", CYN"陷空山无底洞飞天遁地护法"NOR);
		} else if(exp <= 3000000 ){
			me->set("title", CYN"陷空山无底洞偷梁换柱使者"NOR);
		} else if(exp <= 5000000 ){
			me->set("title", CYN"陷空山无底洞来去无踪尊者"NOR);
		} else {
			me->set("title", CYN"陷空山无底洞人肉包子大厨"NOR);
		}
