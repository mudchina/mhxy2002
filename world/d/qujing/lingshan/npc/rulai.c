...
int reward_player ()
{
  ...
  if ((i/4-j) > 0)
  {
    message_vision ("$N对$n摇摇头：梦幻西游现有"+chinese_number(i)+"位玩家，"
                    "此处仅有数位仙贤在邀，可否再邀"+chinese_number(i/4-j)+
                    "位仙贤前来观礼？\n",me,who);
    if (! DEBUG)                 
      return 1;
  }
  announce (me,"梦幻西游西天取真经观礼："+who->query("name")+"大雄宝殿拜礼！\n");
...
很容易会令自身堕入魔道的，你这就去禅房好好参悟刚才的战斗去吧。\n"NOR,me);
        ...
        guai = new("/u/tianlin/rulaitask/"+list[random(sizeof(list))]);
        where=guai->invocation(me, level);
        if(where=="没妖怪。")
        {message_vision(CYN"\n如来佛说道：我没听清，你再问一遍吧！呵呵呵呵。。。。\n"NOR,me);
       
        me->delete("rulaitask/get");
        me->delete("rulaitask/level");
         me->delete("rulaitask/kind");
         destruct(guai);
         return 1;
         }
        switch (kind)
          {
                case "杀": {message_vision(CYN"\n如来佛说道：现在有一个"+where+NOR+CYN"附近出没，此怪
已经残杀了不少无辜，恶贯满盈了，你这就去把它除掉吧。\n"NOR,me); break;}
                case "擒": {message_vision(CYN"\n如来佛说道：现在有一个"+where+NOR+CYN"附近出没，此怪尚未恶
贯满盈，但极难点化，你这就去把它生擒回来，让我亲自点化它。\n"NOR,me); break;}
                case "降": {message_vision(CYN"\n如来佛说道：现在有一个"+where+NOR+CYN"附近出没，此怪刚刚道满出山
尚未为恶，但非常好勇斗狠，你只需狠狠教训它一回，它就不敢为恶了。\n"NOR,me); break;}
                case "劝": message_vision(CYN"\n如来佛说道：现在有一个"+where+NOR+CYN"附近出没，此
...
{
             object me = this_object();
               if (who->query("rulaitask/kind") == "擒" && ob->query("kind") == "擒")  {
        if (ob->query("id")!="yao guai"){
                 message_vision ("$N对$n摇头。\n",me,who);
                    return 0;
               }
             if( who != ob->query("owner")) {

                 message_vision ("$N对$n叹了口气。\n",me,who);
                    return 0;
               }

                say("如来佛:很好, 你去禅房休息吧。\n");
                who->set("rulaitask/accomplish",1);
                who->set("rulaitask/end_time",time());
                who->delete("rulaitask/get");
        ...
        {
       if ( (string)who->query("xiuluo/give_rulai") == "done"){
       command("say 你已经得到惩罚了!");
       return 0;}
...
int do_cancel()
{
        ...
        if(tt<=time() &&
                    time()<tt+600) {
                message_vision("$N说道：时辰未到，不可放弃，阿米陀佛。\n",
                        this_object(), me);
