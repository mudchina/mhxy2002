...
string zname(object);
int rank_me();
#define DIRNAME "/d/jjf/npc/"

#define MYFILE "/d/jjf/npc/zhangmen.c"

void create()
{
	set_name("大元帅", ({ "zhang men", "zhangmen" }) );
	set("gender", "男性" );
	set("age", 30);
	set("str", 30);
	set("per", 30);
...
void reset_me (object me)
{
	...
	me->set_name("朝廷兵马大元帅", ({ "zhang men", "zhangmen" }) );
	me->set("gender", "男性" );
...
int accept_fight(object ob)
{
	...
	if (ob->query_temp("jjf_zm_applied"))
	{
...
int check_result(object me, object ob)
{
	...
	if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 )
	{
		...
		if (me->query_temp("zhangmen/kill") && ob->query_temp("jjf_zm_applied"))
		{
			message_vision ("$N皱了皱眉道：此次比武我未能静心尽力，希望重新来过。\n", me);
			return 1;
		}

		if( ob->query_temp("jjf_zm_applied") != 1 )
			return 1;	
...
{
	message("shout", HIC"〖"HIW"兵马元帅"HIC"〗秦琼(Qin qiong)："HIY"今日是本朝"NOR+RED"兵马大元帅"NOR+YEL+this_player()->name()+HIY"上任的日子，还请诸位江湖朋友多多捧场。\n"NOR,users());
	ob->delete_temp("jjf_zm_applied");
	me->set("new_player",1);
...
string zname(object ob)
{
	if( (string)ob->query("gender") == "女性" ) 
		return HIY"大唐天下"NOR+RED"兵马大元帅"NOR;
	else return HIY"大唐天下"NOR+RED"兵马大元帅"NOR;
}
int rank_me()
{
	...
	exp=exp+dx;
        if( me->query("title") == HIY"大唐天下"NOR+RED"兵马大元帅"NOR)
		return notify_fail("你已经是掌门身份，还来要什么职位．\n");
	if( me->query("family/family_name") == "将军府")
	{
                if( exp <= 40000 ){
			me->set("title", HIY"将军府后备役新兵"NOR);
		} else if(exp <= 150000 ){
			me->set("title", RED"将军府前锋营参将"NOR);
		} else if(exp < 500000 ){
			me->set("title", HIG"将军府神机营带刀校尉"NOR);
		} else if(exp <= 800000 ){
			me->set("title", HIC"将军府前锋营统领"NOR);
		} else if(exp <= 1500000 ){
			me->set("title", RED"将军府"YEL"铁骑营神风骑士"NOR);
		} else if(exp <= 3000000 ){
			me->set("title", HIY"将军府"NOR+RED"一品金甲"NOR+YEL"锦衣卫");
		} else if(exp <= 5000000 ){
		     /*  if ( exp > exp4){
			    me->set("title","大唐镇东大将军");
			    exp4=exp;
		       }	    
		       else if ( exp > exp3 ){
		       	    me->set("title","大唐镇西大将军");
		       	    exp3=exp;    
		       }
		       else if( exp > exp2){
		       	    me->set("title","大唐镇南大将军");
		       	    exp2=exp;
		       }
		       else if( exp > exp1){
		       	    me->set("title","大唐镇北大将军");
		       	    exp1=exp;
		       }
		       else */me->set("title", "大唐护国将军");
		} else {
		       /*if ( exp > dx4){
			    me->set("title","大唐一品定国公");
			    dx4=exp;
		       }
		       else if(exp > dx3){
		       	    me->set("title","大唐一品卫国公");
		       	    dx3=exp;    
		       }
		       else if(exp > dx2){
		       	    me->set("title","大唐一品建国公");
		       	    dx2=exp;
		       }
		       else if(exp > dx1){
		       	    me->set("title","大唐一品楚国公");
		       	    dx1=exp;
		       }
		       else*/
			me->set("title", HIY"大唐"NOR+RED"一品"NOR+YEL"护国公"NOR);
		}
