...
#define DIRNAME "/d/nanhai/npc/"

#define MYFILE "/d/nanhai/npc/zhangmen.c"

void create()
{
	set_name("大师", ({ "da shi", "dashi" }) );
	set("gender", "女性" );
	set("age", 30);
	set("str", 30);
	set("per", 30);
...
int convert_identity (object me, object ob)
{
	...
	ob->set("zhangmen/title",HIY"普陀山弘法大师"NOR);
	ob->set("title",HIY"普陀山弘法大师"NOR);
	ob->set("family/zm", 1);
	ob->save();
...
string zm_apply()
{
	...
	return "我南海普陀一门虽不倚强为胜，但作为掌门弟子还是不能太弱。\n如有意出掌此位，还请一战。\n";
}
...
void master_announce (object me, object who, object ob)
{
	...
	message("shout", HIC"〖"HIY"弘法大师"HIC"〗观音菩萨(guanyin pusa)："HIY"今日是本门弘法大师"NOR+YEL+this_player()->name()+HIY"上任的日子，还请诸位神仙多多捧场。\n"NOR,users());
...
int rank_me()
{
	...
	exp=(int)me->query("combat_exp");
        if( me->query("title") == HIY"普陀山弘法大师"NOR)
		return notify_fail("你已经是掌门身份，还来要什么职位．\n");
	if( me->query("family/family_name") == "南海普陀山")
	{
                if( exp <= 20000 ){
			me->set("title", BLU"南海普陀山火工"NOR);
		} else if(exp <= 150000 ){
			me->set("title", HIB"南海普陀山入室弟子"NOR);
		} else if(exp < 500000 ){
			me->set("title", CYN"南海普陀山知客僧"NOR);
		} else if(exp <= 800000 ){
			me->set("title", HIC"南海普陀山护寺弟子"NOR);
		} else if(exp <= 1500000 ){
			me->set("title", RED"南海普陀山护法僧"NOR);
		} else if(exp <= 3000000 ){
			me->set("title", HIR"南海普陀山护法罗汉"NOR);
		} else if(exp <= 5000000 ){
			me->set("title", YEL"南海普陀山首座"NOR);
		} else {
			me->set("title", HIY"南海普陀山主持"NOR);
		}
