...
setup();
   ...
   carry_object("/d/lingtai/obj/yuanpinzhu")->wear();
 ...
 if(objectp(me=query_temp("last_damage_from") ))
        {
            ...
            {
                if (inv[i]->query("id")=="zhu" || inv[i]->query("id")=="yuan pinzhu")
                {
                   inv[i]->move(me);
                   message_vision("$N突然发现张天师走的时候拉下一个小袋子你赶紧把他捡起来放在怀中。\n",this_object(),me);
            message("channel:rumor",HIM+"【"HIW"小道消息"HIM"】某人："HIG"听说"HIR+me->query("name")+HIG"拿到传说中的"BLINK+HIY"元牝珠"NOR+HIG"了！\n"+NOR,users());
         }
        }
  }
        message("sound", "\n\n张道陵脸色突变，道：天上点卯，恕不奉陪！\n\n", environment());
