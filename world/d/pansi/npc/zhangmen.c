...
#define DIRNAME "/d/pansi/npc/"

#define MYFILE "/d/pansi/npc/zhangmen.c"
...
string zm_apply()
{
	...
	return "我盘丝洞绝学惊才艳世，但是最难把持的只不过一个”情“字！\n行走世间往往情愫难禁，修为不深是不行的。如有意出掌此位，还请一战。\n";
}
...
int check_result(object me, object ob)
{
	...
	if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 )
	{
		...
		{
			message_vision ("$N躬身对$n说道：恭请见过紫霞仙子。\n",me,ob);
...
void find_master (object me, object ob)
{
	...
	{
		message_vision ("$N见到紫霞仙子低声说了几句话，又抬起头朝$n使了个眼色。\n\n",me,ob);
		message_vision ("$N用哀怜的目光看了看你，不知是喜是忧。\n\n",who);
		message_vision ("$N退下。\n\n",me);
		me->set_leader(0);
		call_out("master_announce",1,me,who,ob);
