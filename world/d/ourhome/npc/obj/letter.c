...
void create()
{
        set_name(HIY+"宝盒"+NOR, ({"bao he", "he"}));
        set("weight", 5);
        if( clonep() )
                set_default_object(__FILE__);
        else {
                set("long","这是千里眼托付你的宝物。\n");
                set("unit", "个");
                set("no_get", 1);
        }
        set("channel_id", "送宝精灵");
...
// 解决找人帮忙送物，或者送别人的宝物的bug
        ...
        if((time() > me->query( "letter/task_time" )) ) {
                ...
                tell_object(me, "你没有及时把宝物送到" + ob->query("name") + "的手中，你的任务失败了！\n");
        } else {
                tell_object(me,ob->name()+"说道：多谢，多谢！\n");
                tell_object(me, "你成功地把宝物送到了" + ob->name() + "的手中。\n");
                ...
                CHANNEL_D->do_channel(this_object(),"sys",NOR+me->name()+"将宝物送给"+ob->name()+"获得"HIR+daoxing(i)+NOR"的道行，"HIY+chinese_number(qn)+NOR"点潜能\n");
