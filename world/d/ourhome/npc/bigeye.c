...
// 没完成任务， 但还在时间允许范围内 
        if( ((int)me->query("letter/task_time")) >  time() ){
                tell_object(me,"你现在的任务是把宝盒送给"+me->query("letter/target")+"\n");
                write("千里眼生气的对你说： 你得把宝盒帮我送到呀。\n");
                return 1;
        } 
// 没完成任务，时间已过，增加busy为惩罚时间。        
        ...
        message_vision("千里眼对$N说：你来得正好，我这儿刚好有一件宝物要送出去。\n",me);
...
// 不能送给exp<500的玩家
        ...
        tell_object(me, "把宝物送给『" + ob[i]->short(1) + HIW +"』你的任务就完成了。\n" NOR);
        message_vision("$N给了$n一个宝盒。\n",this_object(),me);
