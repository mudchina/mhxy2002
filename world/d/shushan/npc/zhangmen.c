...
void create()
{
	set_name("大师兄", ({ "zhang men", "zhangmen" }) );
	set("gender", "男性" );
	set("age", 30);
	set("str", 30);
	set("per", 30);
...
void reset_me (object me)
{
	...
	me->set_name("大师兄", ({ "zhang men", "zhangmen" }) );
	me->set("gender", "男性" );
...
//	}
	...
	return "我蜀山剑派虽不倚强为胜，但作为掌门要多处照顾各位师弟师妹，\n不强是不行的。如有意出掌此位，还请一战。\n";
}
...
int check_result(object me, object ob)
{
	...
	if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 )
	{
		...
		{
			message_vision ("$N躬身对$n说道：恭请见过剑圣。\n",me,ob);
...
void find_master (object me, object ob)
{
	...
	{
		message_vision ("$N见到剑圣低声说了几句话，又抬起头朝$n使了个眼色。\n\n",me,ob);
