...
void create()
{
	set_name("碧玉琵琶", ({"biyu pipa", "hammer", "pipa", "fabao"}));
	set_weight(8000);
	if(clonep())
...
int ji_ob(object victim)
{
	...
	message_vision(HIC"\n$N大声叫道“好！且听这一曲如何！”，言罢口念咒文，轻拨弦声。。。\n"NOR, me, fabao);
	message_vision(HIC"只见$n「呼」地一下飞到半空，叮叮铛铛之声不绝于耳，径向$N逼去。\n"NOR, victim, fabao);
...
//...weiqi
	...
	if( random(vic_dx/50) > my_spi ) // /1000 is to convert vic_dx to years
	{
		message_vision(HIC"\n$N哈哈大笑，似乎丝毫不受$n影响。\n"NOR, victim, fabao);
	}
	else
	{//now 躲闪 does not happen, try 接收?
		...
		{
			message_vision(HIC"\n$N大喝一声“收！”，只听叮铛之声大作！但$n的琴音一高再高之后终于乖乖地飞到了$N手中。\n"NOR, victim, fabao);
			fabao->move(victim);
		}
		else
		{//now a hit should happen, let's consider damage
			damage = 0;
			damage = 1000 + my_spi/5 - random(vic_mana/10) - vic_enchant*2;
			...
			if( damage > 0 )
			{
				...
				victim->receive_wound("sen", damage/20);
