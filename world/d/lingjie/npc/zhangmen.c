// zhangmen.c 掌门 
//modified for lingjie by LitChi
...
#define DIRNAME "/d/lingjie/npc/"

#define MYFILE "/d/lingjie/npc/zhangmen.c"
...
void init()
{
	...
	me->create_identity("/d/lingjie/npc/mingwang1", "/d/lingjie/zmroom");
...
//	me->set("title", zname(me)); //for lingjie only, others use above.
...
int convert_identity (object me, object ob)
{
	...
	ob->set("zhangmen/title", zname(ob)); //for lingjie only, others use above.
	//ob->set("title",ob->query("family/family_name")+zname(ob));
	ob->set("title", zname(ob)); //for lingjie only, others use above.
	ob->save();
	...
	me->set("title", zname(ob)); //for lingjie only, others use above.
	me->set("short",me->query("name")+"("+capitalize(me->query("id"))+")");
...
string zm_apply()
{
	...
	ob->set_temp("lingjie_zm_applied", 1);
...
int accept_fight(object ob)
{
	...
	if (ob->query_temp("lingjie_zm_applied"))
	{
...
int check_result(object me, object ob)
{
	...
	if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 )
	{
		...
		if (me->query_temp("zhangmen/kill") && ob->query_temp("lingjie_zm_applied"))
		{
			message_vision ("$N皱了皱眉道：此次比武我未能静心尽力，希望重新来过。\n", me);
			return 1;
		}

		if( ob->query_temp("lingjie_zm_applied") != 1 )
			return 1;	
		...
		{
			message_vision ("$N躬身对$n说道：恭请见过灵主。\n",me,ob);
...
void find_master (object me, object ob)
{
	...
	{
		message_vision ("$N见到降世明王低声说了几句话，又抬起头朝$n使了个眼色。\n\n",me,ob);
...
void master_announce (object me, object who, object ob)
{
	...
	ob->delete_temp("lingjie_zm_applied");
	me->set("new_player",1);
