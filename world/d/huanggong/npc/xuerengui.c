//By tianlin@mhxy for 2001.9.18修改
...
if (!ob->query("killer") ||ob->query("killer") != who->query("id"))
    ...
    message("channel:chat",NOR+HIR+"【大唐天朝】"NOR+HIY"薛仁贵(Xue rengui)："NOR+YEL+who->query("name")+HIY"杀死了"NOR+HIR+who->query("bad/name")+HIY"，功加一等。\n"+NOR,users());
      who->delete("bad");
      return 1;
}
int ask_cancel()
{
        ...
        {
                tell_object(me,"你已经成功了完成守城任务还退什么？\n");
                return 1;
        }
        if(!me->query("job/sxy"))
        {
                tell_object(me,"你没有去守城别谦虚了！\n");
                return 1;
        }
        tell_object(me,"没有完成？好吧！继续努力。\n");
...
{      
    ...
    if(me->query("office_number")>1000||me->query("combat_exp")>2000000)
    {
        tell_object(me,"你还守城？太大材小用了！去天庭立功去吧！\n");
         return 1;
         }
    message_vision(CYN"薛仁贵对$N说道：你这就去吧，为朝廷效力，乃升官捷径也。\n"NOR,me); 
        me->set("job/sxy", 1);
...
{      
    ...
    message_vision(CYN"薛仁贵对$N说道：该奖励你" + exp + "点经验，" + dx + "点道行, " + pot + "点潜能，" + money + "两银子，"+gong + "点官值。\n"NOR,me);        
