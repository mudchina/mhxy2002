...
void create()
{
        set_name(MAG"紫晶石"NOR, ({"zijing shi", "baijing", "shi"}));
        set_weight(1);
        if( clonep() )
...
int do_combine()
{
        ...
        if ( !(d=present("bai shi",me)))
                return notify_fail("你将手中水晶石摆弄了几下，却一无所获，大概还没有收集够。\n");
        if ( !(e=present("chengjing shi",me)))
                return notify_fail("你将手中水晶石摆弄了几下，却一无所获，大概还没有收集够。\n");
        if ( !(f=present("lanjing shi",me)))
                return 
notify_fail("你将手中水晶石摆弄了几下，却一无所获，大概还没有收集够。\n");
        if ( !(g=present("lvjing shi",me)))
                return notify_fail("你将手中水晶石摆弄了几下，却一无所获，大概还没有收集够。\n");
        if ( !(b=present("hongjing shi",me)))
                return notify_fail("你将手中水晶石摆弄了几下，却一无所获，大概还没有收集够。\n");

        if( real=new("/d/longzhu/obj/7")){
                destruct(b);
                destruct(c);
                destruct(d);
                destruct(e);
        ...
        message_vision("$N将手中水晶石按天罡之阵摆好，只见几道霞光掠过，七颗水晶石居然合为一体。\n",me);
        destruct(a);
        }
        return 1;
}
