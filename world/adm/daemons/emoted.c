...
if(objectp(target)) {
 if(!living(target)) write(HIW"但是很显然，"+target->name(1)+HIW"现在的状态没法给你任何答复。\n"NOR);
 else if (interactive(target)&&query_idle(target)>300) write(YEL+"但是"+target->name(1)+NOR+YEL"已经发呆"HIR+chinese_number(query_idle(target)/60)+NOR YEL"分钟，看来不会有反应了！\n"NOR);
