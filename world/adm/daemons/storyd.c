...
//By tianlin@mhxy for 2002.1.9.从新改编,加入大量故事
#include <ansi.h>
...
void ready_to_start()
{
    ...
    //    call_out("start_story", 10);//间隔时间
        call_out("start_story", 3600 + random(300));//间隔时间
...
void process_story(object ob)
{
	...
	object *listeners;
        ...
        prompt = ob->prompt();
        if (! prompt) prompt = HIC"【"BLK"梦幻逸事"HIC"】" NOR;
...
/*
	...
	sprintf("听说%s被扔到了%s。\n", 
                               ob->name()+HIM, env->query("short")+HIM, env));
  */      }
}
