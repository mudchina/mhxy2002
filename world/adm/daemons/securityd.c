...
private mapping exclude_write = ([/*不能写*/
    ...
    "log":      ({ "(admin)","(arch)","(wizard)","(apprentice)","(immortal)",}),
    "cmds":     ({ "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "std":      ({ "(apprentice)","(wizard)",}),
    "d" :       ({ "(apprentice)","(immortal)",}),
    "u":        ({ "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "d/newjob": ({ "(arch)","(wizard)","(apprentice)","(immortal)",}),
]);

private mapping exclude_read = ([/*不能读*/
    "adm":       ({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "d/newjob":  ({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "clone":     ({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "daemon":    ({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "quest":     ({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "data":      ({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "binaries":  ({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "include":   ({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "feature":   ({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "u":         ({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "std":       ({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "daemon":    ({  "(wizard)","(apprentice)","(immortal)",}),
    "log"    :   ({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "topten" :   ({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "backup" :   ({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "cmds/adm":  ({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "cmds/arch": ({  "(wizard)","(apprentice)","(immortal)",}),
    "d/obj/drug":({  "(arch)","(wizard)","(apprentice)","(immortal)",}),
    "d":         ({  "(immortal)",}),
]); 
...
string get_status(mixed ob)
{
   ...
   if(euid == "tianlin") return "(admin)";
   if( !undefinedp(wiz_status[euid]) ) return wiz_status[euid];
   else if( member_array(euid, wiz_levels)!=-1 ) return euid;
   else return "(player)";
}
string get_chinese_status(mixed cob)
{
   string ceuid;
   if( objectp(cob) ) {
     ceuid = geteuid(cob);
     if( !ceuid ) ceuid = getuid(cob);
   }
   else if( stringp(cob) ) ceuid = cob;
   if (!mapp(wiz_status)) return CYN"普通玩家"NOR;
   if( !undefinedp(wiz_status[ceuid]) ) return HIC"梦幻西游巫师"NOR;
   else if( member_array(ceuid, wiz_levels)!=-1 ) return ceuid;
   else return CYN"普通玩家"NOR;
}
...
//xlwang  11.29 2000
//判断ip是否符合okip
int match_site(object ob, string site)
  {
  int i;
  string* ok_ip;
  string* line; 
  ok_ip=ob->query("ok_ip");
  if (!ok_ip) return 1;
  line = ({});
  line += ({site});
  if(!pointerp(ok_ip)) ok_ip=line;
  for (i=0;i<sizeof(ok_ip);i++) 
  if(sizeof(regexp(line, ok_ip[i])) == 1 ) return 1;
return 0;
}
...
//        if( sscanf(file, "/u/" + euid + "/%*s") )
	if( sscanf(file, "/u/" + euid + "/%*s") || file=="/u/"+euid)
                return 1;
