...
//	if (!old) DNS_MASTER->set_mud_info(name, nnew);
	...
	// if this is a new entry, we try to ping the mud
	// and only update our database from the result of ping.
	// mon 7/8/98
	if (!old) {
	    if(!undefinedp(nnew["HOSTADDRESS"]) &&
		    !undefinedp(nnew["PORTUDP"]) )
		    PING_Q->send_ping_q(nnew["HOSTADDRESS"], nnew["PORTUDP"]);
	}

	// We don't want to update an old mud info 
	// simply because
