...
void send_ping_query(string host, mixed port);
...
{
/*        CHANNEL_D->do_channel(this_object(), "sys", "ping: "+
	  info["NAME"]+" "+info["HOSTADDRESS"]+" "+
	  info["PORTUDP"]+"(to "+Mud_name()+")");
...
// send a pinq query
void send_ping_q(string host, mixed port, int no_delay)
{
    // delay to prevent others flood us with ping.
    if(!no_delay)
      call_out("send_ping_query", 1, host, port);
    else
	send_ping_query(host, port);
}

// this should only be called by send_ping_q
// mon 7/8/98
void send_ping_query(string host, mixed port)
{
