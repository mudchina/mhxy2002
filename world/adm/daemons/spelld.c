...
int attacking_cast_damage(object attacker, object target, int damage_adj)
{
	...
	a_fali+=((2*damage_adj/100)*(attacker->query("mana_factor"))*1)/1;
	d_fali+=((2*damage_adj/100)*random(target->query("mana_factor"))*1)/1;
	...
	// if backfire<0 then backfired.
	// if a_fali==d_fali, 1/4 chance backfire.
	if(random(100)<(100*a_fali/(a_fali+d_fali/3)))
	    backfire=1;
	else
	    backfire=-1;
	// damage is mostly determined by the attacker.
	damage=a_fali/2;

	if( backfire < 0 ) {
	    //for backfire, we need let it be serious...
	    //here increase the damage, 
	    //since previously it's included in random()
	    damage = damage + target->query("mana_factor"); 
	    // backfire, not direct hit, so decrease damage.
	    damage=damage/3;
	}
	...
	//the setting is, at enabled level 400, the damage will double.
	if( backfire >= 0 ) {
	    damage += (damage*(int)attacker->query_skill("spells"))/400;
	} else {
	    damage += (damage*(int)target->query_skill("spells"))/400;
	}
	
	if(damage>1000) damage=1000;
	if(backfire<0) damage=-damage;
...
//	Check Victim cast lianhua
	if (victim->query_temp("daoqian")==1 && victim->query("mana")>=500)
	{
		damage=(int)(damage/3);
		victim->add("mana",-500);
		msg_hit=msg_hit+"\n"WHT"只见$n背后幻出"NOR+BLINK+HIR"千朵莲花"NOR+WHT",挡住$N的攻势\n"NOR;//this is by tianlin加以描述.2001.5.1
	}	
