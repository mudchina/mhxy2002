...
// by mon 10/29/97. remove call_other, use load_object to find and load.
  ...
  if (silently==2 || this_object() -> query("env/brief_all"))
   ...
   str = sprintf( "%s - %s\n  %s%s",
    environment()->query("short")?(environment()->query("outdoors")?HIR"「"NOR+WHT+(string)environment()->query("short")+NOR+HIR"」"NOR:HIR"「"NOR+HIG+BLK+U+(string)environment()->query("short")+NOR+HIR"」"NOR): "",
  ...
  // modified by mon 8/29/97
    ...
    file_name(this_object())[0..2]=="/ww/") 
    && userp(ob))
{      MONITOR_D->report_system_object_msg (ob,"得到了"+
       this_object()->query("name")
    +"("+file_name(this_object())+")。");
}
//By tianlin@Mhxy for 2002.1.24加入
 if (this_object()->query("rumor_msg")&&userp(ob)&&!wizardp(ob))
     CHANNEL_D->do_channel(this_object(), "rumor",
sprintf("听说"NOR+WHT"%s"HIM"在"HIW"%s"HIM"附近的"NOR+RED"%s"HIM"里得到了传说中的"+HIY"%s"+HIM+",不知是真是假。\n",ob->name(1),MISC_D->find_place(environment(this_player())),environment(this_player())->query("short"),this_object()->name(1)));
