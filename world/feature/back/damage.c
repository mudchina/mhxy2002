...
/*	if(userp(this_player())&&this_player()->query("age")<=16) 
	{
        this_player()->set("eff_kee",this_object()->query("max_kee"));
	 ...
	 message_vision(HIR "忽然，田林出现在烟雾之中。说道：不准残害未成年儿童！\n" NOR, this_object());
...
*/
                        ...
                        if ( guai == me->query_temp("last_damage_from") || guai == me->query_temp("last_opponent") )
                             {
                                ...
                                message_vision(HIY"如来佛一伸左手把$N的魂魄从地府里拉了回来，再一伸右手不知道从哪里把$N的尸体抓了回来，接着两手一合，硬生生地把$N魂魄逼回身体。\n"NOR,me);
                                me->unconcious();
                                return;
                              }
                 }
	...
	if( userp(this_object()) ) {
		...
		LOGIN_D->add_dienpc();              //浩劫系统的触发条件  add by xhm 2000.11.13
		if ( LOGIN_D->get_madlock() )       //每死一个NPC就判断浩劫是否被平息
			if ( query("haojie") ) !F_DISASTER->verdict_end();
