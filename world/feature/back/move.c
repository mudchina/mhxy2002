...
// by mon 10/29/97. remove call_other, use load_object to find and load.
  ...
  if (silently==2 || this_object() -> query("env/brief_all"))
   ...
   str = sprintf( "%s - %s\n  %s%s",
    environment()->query("short")?(environment()->query("outdoors")?HIR"「"HIG+(string)environment()->query("short")+HIR"」"NOR:HIR"「"HIY+(string)environment()->query("short")+HIR"」"NOR): "",
...
//added by hawks 2000/12/ 
 if (this_object()->query("rumor_announce")&&userp(ob)&&!wizardp(ob))
     CHANNEL_D->do_channel(this_object(), "rumor",
        sprintf("听说%s得到了传说中的"+HIY"%s"+HIM+",不知是真是假。\n",ob->name(1),this_object()->name(1)));
