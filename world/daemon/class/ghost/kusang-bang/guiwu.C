        ...
        if( !target ) target = offensive_target(me);
         if( !target || !target->is_character()
           || !me->is_fighting(target) || !living(target) )
                return notify_fail("「黄泉鬼舞」只能在战斗中使用。\n");
       if( !(weapon1 = me->query_temp("weapon"))
        ||      (string)weapon1->query("skill_type") != "stick" )
                return notify_fail("你以为你是谁？没棒也发「黄泉鬼舞」? \n"); 

        if(me->query("family/family_name")!="阎罗地府")
                return notify_fail("你非地府弟子，无法使用「黄泉鬼舞」。\n");


        if( (int)me->query_skill("stick", 1) < 100 ||
           (int)me->query_skill("kusang-bang", 1) < 100 )
                return notify_fail("你有的功夫还不够娴熟，不会使用「黄泉鬼舞」。\n");
        
        if((int)me->query_skill("dodge", 1) <120)
                return notify_fail("以你现在的轻功修为如何「黄泉鬼舞」。\n");
        
        if((int)me->query_skill("force", 1) <100)
                return notify_fail("以你现在的内功修为还使不出「黄泉鬼舞」。\n");
...
if(target) force_skill = target->query_skill_mapped("force");
   if( !force_skill ) force_skill_name = "基本内功";
...
else                if( force_skill=="guangming-force" ) force_skill_name = "光明心法";
else                if( force_skill=="zhenyuan-force" ) force_skill_name = "镇元神功";
else                if( force_skill=="humoforce" ) force_skill_name = "火魔心法";
        ...
        if( !target || !target->is_character()
              || !me->is_fighting(target) || !living(target) )
            return notify_fail("「黄泉鬼舞现在没对手了呀。\n");
