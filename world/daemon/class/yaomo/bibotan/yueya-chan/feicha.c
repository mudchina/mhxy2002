...
//       "action" :  HIC"$N突然清啸一声，掌中$w"+HIC"脱手而出,直向$n的$l飞去,这正是[飞叉绝艺]!"NOR,
      ...
      if( !target || !target->is_character()||target->is_corpse()||target==me)
          return notify_fail("你要对谁使用"NOR+HIY"["NOR+BLINK+HIW"飞叉绝艺"NOR+HIY"]"NOR"？\n");
      if (!target->is_fighting(me))
             return notify_fail("你们没有在打架！\n");
      if (me->query_temp("no_feicha") )
          return notify_fail("你现在状态不佳，不能使用飞叉绝艺。\n");      
      if (me->query_skill("huaxue-force",50)<70)
          return notify_fail("你的内功差。\n");
      if (me->query_skill("yueya-chan",50)<70) 
    ...
    message_vision(HIC"$N突然清啸一声，手中"+weapon->query("name")+HIC"脱手而出，直向$n飞去！这正是"NOR+HIY"["NOR+BLINK+HIW"飞"HIG"叉"NOR+MAG+BLINK"绝艺"NOR+HIY"]"NOR+HIC"!"NOR,me,target);
