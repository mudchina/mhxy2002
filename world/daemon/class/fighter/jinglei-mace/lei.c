...
int perform(object me, object target)
{
     ...
     if (random(myexp)>(int)(texp/10) && random(myforce) > (int)(tforce/3)) 
        ...
        damage= (int)(((int)me->query("max_force")/2+myforce -  (int)target->query("max_force")/2 - tforce)/4);
     ...
     damage=(random(100)+me->query("force_factor"))/2;
     if(damage<1)damage = 1;
     target->receive_damage("kee", me->query("eff_kee")*damage/80, me);
     target->receive_wound("kee", me->query("eff_sen")*damage/100, me);
     message_vision(msg, me, target);
...
"$n连锏影还没看到，就差点儿被穿的透心凉！\n\n"NOR;
damage=(int)(random((int)(myskill1/2))+me->query("force_factor")/2);
if(damage<1)damage = 1;
target->receive_damage("kee", me->query("eff_kee")*damage/80, me);
target->receive_wound("kee", me->query("eff_sen")*damage/100, me);
     message_vision(msg, me, target);
 COMBAT_D->report_status(target);
                     if (target->query("eff_kee")<0 || !living(target))  
                       {str=GRN+target->name()+HIM"被"NOR+YEL+me->name()+HIM"用"NOR+HIY"将军府"NOR+HIB"绝学"HIR"『"HIC"雷殛怒火"HIR"』"HIM"送去了阎罗地府, 听说尸体被烧的焦黑!";
