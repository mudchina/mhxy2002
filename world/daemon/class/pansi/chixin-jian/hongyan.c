...
int perform(object me, object target)
{
	...
	||	!me->is_fighting(target) )
		return notify_fail(HIR"红颜"HIW"白发"NOR"只能对战斗中的对手使用。\n");
	weapon = me->query_temp("weapon");
	if (!weapon) return notify_fail(HIR"红颜"HIW"白发"NOR"只能和痴心情长剑法配和！\n");
...
return notify_fail("“红颜白发”只有盘丝洞门人才可以用！\n");
	...
	COMBAT_D->do_attack(me, ob, me->query_temp("weapon"), 2);
                     ...
                     if (target->query("eff_kee")<0 || !living(target))  
                       {str=target->name()+HIM"被"+me->name()+HIM"用盘丝洞绝学"HIC"“"HIR"红颜"HIW"白发"HIC"”"HIM"给杀死了!";
