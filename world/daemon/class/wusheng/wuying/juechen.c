...
int perform(object me, object target)
{
        ...
        ||      target->is_corpse()
        ||      target==me)
                return notify_fail("你要对谁施展「绝尘无影」？\n");
        if(!me->is_fighting())
                return notify_fail("「绝尘无影」只能在战斗中使用！\n");
        if(me->query("family/family_name")!="武圣门")
                return notify_fail("不是武圣门的不能用「绝尘无影」！\n");
        ...
        if((int)me->query_skill("wuying", 1) < 100)
                return notify_fail("你的无影剑修为还不够，使用这一招会有困难！\n");

        msg = HIR"\n$N手腕晃动如疾风，以疾风的速度瞬间挥出九千九百九十九剑，剑锋无影直逼$n！\n"NOR;
