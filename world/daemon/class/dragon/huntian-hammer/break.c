...
int perform(object me, object target)
{
	...
	int i, mypot,tapot,temp_dodge,temp_spells;
        ...
        ||      target->is_corpse()
        ||      target==me)
                return notify_fail("你要对谁施展这一招"YEL"「混元一破」"NOR"？\n");

        if(!me->is_fighting())
                return notify_fail(""YEL"「混元一破」"NOR"只能在战斗中使用！\n");
...
if( random( mypot+tapot )*5 > tapot ) {
 ...
 ((int)me->query("max_force")/100+(int)me->query("force_factor"))*3);
			...
			if (target->query("eff_kee")<0 || !living(target))  
                       {str=target->name()+"被"+me->name()+"用龙宫绝招「混元一破」杀死了。听说尸体几乎被砸成了肉饼！哦，可怜的"+str=target->name()+"！";
