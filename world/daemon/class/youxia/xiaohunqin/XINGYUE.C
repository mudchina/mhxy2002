...
object weapon2;
        ...
        if (!me->query("married"))
            return 0;
 if (!victim) return notify_fail("你想对谁进行「星月齐飞」？\n");
 if( !target )
  return notify_fail("你想与谁进行「星月齐飞」？\n");
 if( target->query("id") == me->query("id") )
  return notify_fail("你想与自己进行「星月齐飞」？\n");
 if( victim->query("id") == target->query("id") )
  return notify_fail("不能与你的对手进行「星月齐飞」。\n");
...
if( me->query("gender") != "男性" )
 {
  ...
  if( target->query("name") != me->query("husband/name")&& target->query("id") != me->query("couple/id") )
         return notify_fail("你只能与丈夫进行「星月齐飞」。\n");
 }
 else
 {
  msg = "妻子";
  ...
  if( target->query("name") != me->query("wife/name")&& target->query("id") != me->query("couple/id") )
         return notify_fail("你只能与妻子进行「星月齐飞」。\n");
 }
 if( !victim->is_character() || !me->is_fighting(victim) )
  return notify_fail("只能对你正在战斗中的对手使用「星月齐飞」。\n");
...
if( !objectp(weapon = me->query_temp("weapon")) ||!objectp(weapon2 = target->query_temp("weapon")) )
            return notify_fail("你们空手怎么「星月齐飞」啊？\n");
 if( (int)target->query_temp("xingyue") )
  return notify_fail("你的搭档已经在使用「星月齐飞」了。\n");
 if( (int)me->query_temp("xingyue") )
  return notify_fail("你已经在使用「星月齐飞」了。\n");
...
me->set_temp("xingyue_request", 1);
  tell_object(target, me->query("name")+"向你发出「星月齐飞」请求。\n");
tell_object(me,"你向"+target->query("name")+"发出「星月齐飞」请求。\n");
  return 1;
 }
