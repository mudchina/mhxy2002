...
int perform(object me, object target)
{
   ...
   int wound=0;
      ...
      req=(int)me->query_skill("sword",50)+(int)me->query_skill("yatu-sword", 50)+(int)me->query_skill("throwing", 1); 
      ...
      if (me->query_skill_mapped("force")!="zanglong-force")
                return notify_fail("幻影飞针必须配合藏龙心法才能使用。\n");
      if(req < 80)
            return notify_fail("你剑中打暗器的功夫还不到火候！\n");
      if(req < 150 && weapon->query("anqi/now") ==0)
            return notify_fail("你剑中没有暗器可打！\n");
       if (me->query("force")<300)
           return notify_fail("你内力不继，难以幻影飞针。\n");

  if (me->query_temp("huanying_busy")) return notify_fail("出奇才能制胜，幻影飞针多使就不灵了。\n");
        me->add("force", -200);
        ...
        msg = MAG"只见$N手中"+weapon->query("name")+MAG"急颤，幻出满天剑芒，无数"
                 +anqi_type+MAG"幻影般向$n扑来！\n" NOR;
...
*/
    ...
    if (random(damage)>target->query_temp("apply/armor"))  {
       ...
       wound=1;
           ...
           msg += MAG "$n躲闪不及，身上"+chinese_number(hitt)+HIW "处中了"+anqi_type+"！\n"NOR;
           message_vision(msg, me, target);
     ...
     COMBAT_D->report_status(target,wound);
        ...
        msg = CYN "$n对着$N高声骂道：好厉害的招数，看我怎么收拾你！\n" NOR;     
        message_vision(msg, me, target);
        target->kill_ob(me);
      me->set_temp("huanying_busy",3);
