...
message("vision",HIY+"【朝廷大政】:"+NOR+HIC+me->query("name")+
...
me->add("soilder",-pts);
message("vision",HIY+"【朝廷大政】:"+NOR+HIC+me->query("name")+"派驻了"+chinese_number(pts)+
...
message("vision",HIY+"【战火纷飞】:"+NOR+HIC+me->query("name")+"派了"+chinese_number(pts)+
...
message("vision",HIY+"【战火纷飞】:"+NOR+HIC+me->query("name")+"取消了对"+this_object()->query("short")+
"的进攻，自动退兵了.\n"NOR,users());
save();
return 1;
}
...
message("vision",HIY+"【朝廷大政】:"+NOR+HIC+me->query("name")+"自动放弃了对"+this_object()->query("short")+
"的统治，撤离了该城.\n"NOR,users());
save();
return 1;
}
