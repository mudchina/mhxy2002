...
int do_change(string arg)
{
  ...
  if (arg) {
    if(arg=="vikee") return notify_fail("我看还是算了吧!一叶知秋可不是好惹的,把他惹急了可不是闹着玩的!\n");
...
int do_hun (string str)
{
  ...
  if (str=="vikee") return notify_fail ("想死的话用suicide -f命令方便一点!\n");
  who = LOGIN_D->find_body(str);
  if (!who){ 
  if (!(who=present (lower_case(str), environment(this_player()))) )
    return notify_fail ("这里没有"+str+"\n");
...
int do_disable_player (string str)
{
  ...
  if (str=="vikee") return notify_fail("这可不是闹着玩的!\n");
