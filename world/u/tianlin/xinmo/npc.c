...
void copy_status(object me,object ob)
{
        ...
        me->set_name(ob->query("name")+HIC"的"NOR+MAG"心魔"NOR,({ob->query("id")+" npc","npc"}));
        me->set("gender",ob->query("gender"));
        me->set("title",ob->query("title"));
        me->set("enable_type","挑衅者");
...
void die()
{
        ...
        ob->add("daoxing",reward*2);
        ob->add("potential", reward*2/3);
        ob->add("zhongcheng", 1);
        ob->add("officer_number", 1);
      switch(random(2))
           {  case 0:
        CHANNEL_D->do_channel( this_object(), "rumor",
sprintf("挑衅%s的"NOR"「"YEL+"%s」"NOR+HIM"被"NOR+HIR"%s"NOR+HIM"给灭了。"NOR,query("target"),query("name"), ob->name() ));
              break;
              case 2:
             CHANNEL_D->do_channel( this_object(), "rumor",
sprintf("挑衅%s的"NOR"「"YEL+"%s」"NOR+HIM"被"NOR+HIR"%s"NOR+HIM"送上西天了。"NOR,query("target"),query("name"), ob->name() ));
              break;
              case 1:
        message("system",HIC"【"NOR+HIW"门派"NOR+HIC"】"NOR+CYN+ob->query("family/family_name")+"掌门(Zhangmen)："+"本门爱徒"NOR+HIM+ob->name()+NOR+CYN"把挑衅"+query("target")+"的"NOR+query("name")+CYN"给灭了，成功保护了本门派的安全，受到本门派的嘉奖！\n"NOR,users());
            break;
           }
        tell_object(ob,"\n你得到了"+daoxing(reward)+"道行和"
                + chinese_number(reward/2) + "点潜能！\n" );
        tell_object(ob,"你的门派忠诚度和官值增加了。\n");
        MONITOR_D->report_system_object_msg(ob,"得到了"+daoxing(reward)+"道行和"
                + reward/2 + "点潜能！");
        }
        ::die();
}
