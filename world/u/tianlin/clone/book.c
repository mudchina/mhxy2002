...
void create()
{
        set_name(WHT"¸ñ¶·ÌìÊé"NOR, ({"book"}) );
        set_weight(10);
        set("no_steal", 1);
        set("no_beg", 1);
        set("no_sell", 1);
        set("no_give",""HIW"±ù¡¡Îè¡¡·É¡¡Ñ©"HIM"ÀäÀäµØ£¬ÈáÈáµØ´Ó¿ÕÖĞÆ®È»¶øÖÁ¡£"NOR"ËıÀ­×¡ÄãµÄÊÖ£¬ËµµÀ£º¡°Ğ¡ÅóÓÑ£¬
ÌìÊé¿ÉÊÇ²»ÄÜËæ±ãËÍÈËµÄÅ¶¡£¡±×ªÑÛ¼û"HIW"Ëı"HIM"ÒÑ¾­ÏûÊ§ÔÚÀäÀäµÄÔÂ¹âÖĞ¡£\n\n");
        set("no_get", 1);
        set("no_drop",""HIW"±ù¡¡Îè¡¡·É¡¡Ñ©"HIM"ÀäÀäµØ£¬ÈáÈáµØ´Ó¿ÕÖĞÆ®È»¶øÖÁ¡£"NOR"Ëı¶ÔÏòÄãµÄÆ¨Æ¨ÉÏÓÃÁ¦µÄÌßÁËÒ»½Å£¬
ºÜÉúÆøµÄËµ£º¡°ºÃÍÛ£¬ÄãÕâ¸öÃ»Á¼ĞÄµÄ£¬ÎÒËÍÄãµÄ±¦±´Äã¶¼¸ÒÈÓ°¡£¿Ëµ°Õ"HIW"±ù¡¡Îè¡¡·É¡¡Ñ©"HIM"ÒÑÏûÊ§ÔÚÀäÀäµÄÔÂ¹âÖ®ÖĞÁË¡£\n\n");
        set("long",@LONG
ÕâÊÇÃÎ»ÃÎ÷ÓÎµÄÎ×Ê¦ÖÆ×÷µÄÒ»±¾ÌìÊé,ËüµÄ×÷ÓÃ¿É´óÁË,ÓĞÒâÕßÇë(kan)¾ÍĞĞÁË!
LONG );
        if( clonep() )
...
¾ÈÃü´ó·¨(xfull)       Á¢Âí»Ö¸´¾«ÉñÌåÁ¦£½Î×Ê¦µÄ»Ö¸´Ö¸Áî\n

½î¶·ÔÆ(flyto)         µ½ÏëÈ¥µÄµØ·½£½Î×Ê¦µÄgotoÖ¸Áî\n");
        return 1;
}
