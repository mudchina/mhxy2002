...
void create()
{
  ...
  set("inquiry", ([
    "here":      "此乃禹王林，妖魔大会在此定期("HIR"schedule"NOR")举行也。\n",
    "?":         "此乃禹王林，妖魔大会在此定期("HIR"schedule"NOR")举行也。\n",
...
{
    string str;
    str=HIR+chinese_number(year)+HIY"年妖魔大会"HIC;
//    if(str[0..1]=="一")
//	str=str[2..];
    return str;
}
...
int check_players ()
{
  ...
  while (j--) {
    ...
    if (! players[nameids[j]]) {
      i--;
      announce(HIR+nameids[j]+HIC"临阵脱逃，退出大会。\n");
...
//      level=get_level()+1;
	  ...
	  room->reset_watch_list();
          //announce(title+"将于"HIR+chinese_number(INTERVAL/3600)+HIC
          announce(title+"将于"HIR+chinese_number((NEXT_GAME+1800)/3600)+HIC
		  "小时后举行！\n"NOR);
      }
    } else {
      winner=({});
      ...
      if(players) {
	  announce(title+"第"HIR+chinese_number(MAXROUND-round)+HIC
...
//	  room->delete("players");
      ...
      env->set("players",([]));
      announce(title+"第"HIY+chinese_number(MAXROUND-round+1)+HIC
	      "场时间用尽，不分胜负！\n"NOR);
      env->checks();
...
//	message_vision("$N又补充到：您过"+str+"再来吧。\n",me);
  ...
  if (query_status() == INVITE) {
    ...
    announce (title+"将于"HIR"一分种"HIC"后开始！玩家准备入场就位！\n"NOR);
    set_status(DELAY);
...
int do_join (string arg)
{
  ...
  message_vision ("$N说道：请使"HIR"init"NOR"命令也。\n",me,who);
  return 1;
}
...
{
  int i = bonus;
  announce (title+"第"HIR+chinese_number(MAXROUND-round+1)+HIC
	  "场结束！嬴家："HIY+
	  (who?who->query("name"):"(无)")+HIC"！\n"NOR);
...
//      level=get_level()+1;
  ...
  if (who) {
      ...
      announce("奖"+who->query("name")+"一"+
	      ob->query("unit")+ob->name()+HIC"及"HIY+
	      chinese_number(kill_bonus)+HIC"点潜能！\n"NOR);
