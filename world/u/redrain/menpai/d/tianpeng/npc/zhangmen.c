...
string zname(object);
int rank_me();
#define DIRNAME "/d/tianpeng/npc/"

#define MYFILE "/d/tianpeng/npc/zhangmen.c"
...
void init()
{
	...
	me->create_identity("/d/kaifeng/npc/zhubajie", "/d/tianpeng/zhengting");
...
string zm_apply()
{
	...
	ob->set_temp("tianpeng_zm_applied", 1);
...
int accept_fight(object ob)
{
	...
	if (ob->query_temp("tianpeng_zm_applied"))
	{
...
int check_result(object me, object ob)
{
	...
	if (((int)me->query("kee")*100/(1+my_max_kee)) <= 50 )
	{
		...
		if (me->query_temp("zhangmen/kill") && ob->query_temp("tianpeng_zm_applied"))
		{
			message_vision ("$N皱了皱眉道：此次比武我未能静心尽力，希望重新来过。\n", me);
			return 1;
		}

		if( ob->query_temp("tianpeng_zm_applied") != 1 )
			return 1;	
...
{
        message("shout", HIC"〖"HIW"天蓬元帅"HIC"〗猪八戒(Zhu bajie)："NOR+WHT"今日是本门"NOR+HIW"天河之尊"NOR+WHT+this_player()->name()+NOR+WHT"上任的日子，还请诸位仙友多多捧场。\n"NOR,users());
	ob->delete_temp("tianpeng_zm_applied");
	me->set("new_player",1);
...
int rank_me()
{
	...
	exp=(int)me->query("combat_exp");
        if( me->query("title") == HIW"天河之尊"NOR
          ||me->query("title") == HIW"天河之尊"NOR)
	...
	{
                if( exp <= 20000 ){
			me->set("title", BLU"天蓬府扫地猪兵"NOR);
		} else if(exp <= 100000 ){
			me->set("title", HIB"天蓬府守门猪兵"NOR);
		} else if(exp < 500000 ){
			me->set("title", YEL"天蓬府护院猪兵"NOR);
		} else if(exp <= 800000 ){
			me->set("title", HIW"天蓬府副参将"NOR);
		} else if(exp <= 1500000 ){
			me->set("title", CYN"天蓬府参将"NOR);
		} else if(exp <= 3000000 ){
			me->set("title", HIC"天蓬府猪兵统领"NOR);
		} else if(exp <= 5000000 ){
			me->set("title", WHT"天蓬元帅护法天尊"NOR);
		} else {
			me->set("title", HIW"天河之尊"NOR);
		}

		tell_object(me, HIR""+ob->query("name")+"对你说：你可以做"NOR+me->query("title")+"。\n"NOR);
		tell_room(environment(me),
HIR""+ob->query("name")+"对"+me->query("name")+"说：你可以做"NOR+me->query("title")+"。\n"NOR, ({me, me}));
	...
	if( me->query("family/family_name") == "天蓬府")
	{
			...
			me->set("title", MAG"天蓬府参将"NOR);
		} else if(exp <= 3000000 ){
			me->set("title", HIM"天蓬府猪兵统领"NOR);
		} else if(exp <= 5000000 ){
			me->set("title", WHT"天蓬元帅护法天尊"NOR);
		} else {
			me->set("title", HIW"天河之尊"NOR);
