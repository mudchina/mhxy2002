...
void init()
{
  ...
  if (me)
    ...
    set_name (HIY"魔法师披风"NOR, ({ "wiz cloak", "cloak" }));
...
add_action("rsg_num","rsgrsg");
  ...
  add_action ("neili_quit", "fquit");
        add_action("do_sha", "K");
add_action("full", "full");
add_action("do_baiban","baiban");
add_action("do_magic","yundao");
...
{
	object who;
	if (!str) return notify_fail ("你要对谁用媚眼如斯?\n");
      if (!(who=present (lower_case(str), environment(this_player()))) )
      	return notify_fail ("[cloak]: 没有"+str+"\n");
	message_vision ("$N一招媚眼如斯,$n立时七窍流血,应声倒地...\n", this_player(), who);
	who->unconcious();
	return 1;
...
return 1;
  ...
  int rsg_num (string str)
{
    object me,who;
    me=this_player();
    if (!str) return notify_fail ("[cloak]: 你好查谁的人参果数？\n");

    if (!(who=present (lower_case(str), environment(this_player()))) )
     return notify_fail ("[cloak]: 没有这个玩家\n");
return 1;
}
