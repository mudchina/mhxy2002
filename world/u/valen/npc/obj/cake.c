...
"可以许愿要经验(exp)、潜能(pot)、道行(dx)、法力(mana)、黄金(gold)或者内力(force)。\n");
       return 1;
   } 
  me->set("food", (int)me->max_food_capacity());
  me->set("water", (int)me->max_water_capacity()); 
...
int do_wish(string arg) 
{
  ...
  return notify_fail("你想许什么愿？可以许愿要经验(exp)、潜能(pot)、道行(dx)、法力(mana)、黄金(gold)或者内力(force).\n"); 
}
